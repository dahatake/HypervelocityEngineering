...
```mermaid
flowchart TD
  s1_1(["Arch-Batch-DomainAnalytics [1.1]"])
  s1_2(["Arch-Batch-DataSourceAnalysis [1.2]"])
  s2(["Arch-Batch-DataModel [2]"])
  s3(["Arch-Batch-JobCatalog [3]"])
  s4(["Arch-Batch-ServiceCatalog [4]"])
  s5(["Arch-Batch-TestStrategy [5]"])
  s6_1(["Arch-Batch-JobSpec [6.1]"])
  s6_2(["Arch-Batch-MonitoringDesign [6.2]"])
  s6_3(["Arch-Batch-TDD-TestSpec [6.3]"])
  s1_1 -->|"docs/catalog/use-case-catalog.md"| s2
  s1_2 -->|"docs/catalog/use-case-catalog.md"| s2
  s2 -->|"docs/batch/batch-domain-analytics.md"| s3
  s2 -. "docs/batch/batch-domain-analytics.md（代替依存）" .-> s3
  s3 -->|"docs/batch/batch-domain-analytics.md"| s4
  s3 -. "docs/batch/batch-domain-analytics.md（代替依存）" .-> s4
  s4 -->|"docs/batch/batch-job-catalog.md"| s5
  s4 -. "docs/batch/batch-job-catalog.md（代替依存）" .-> s5
  s5 -->|"docs/batch/batch-service-catalog.md"| s6_1
  s4 -. "docs/batch/batch-job-catalog.md（代替依存）" .-> s6_1
  s5 -->|"docs/batch/batch-service-catalog.md"| s6_2
  s4 -. "docs/batch/batch-job-catalog.md（代替依存）" .-> s6_2
  s6_1 -->|"docs/batch/batch-service-catalog.md"| s6_3
  s6_2 -->|"docs/batch/batch-service-catalog.md"| s6_3
...
[Bootstrap Workflow]
    ├─ ラベル自動作成
    ├─ Step.1.1〜6.3 の Sub-issue 一括生成
    ├─ step-1.1 & step-1.2: abd:ready + abd:running + Copilot assign（並列開始）
    └─ step-2〜6.3: ラベルなし（依存先完了待ち）

[Step Issue abd:done ラベル付与または close]
    └─ [auto-batch-design.yml 状態遷移] → 依存関係チェック → 次 Step を abd:ready + abd:running + Copilot assign
...
GitHub Copilot には複数のAgentがあります。ご自分の好みのものを選択してください。
...
2. `.github/workflows/auto-batch-design.yml` がリポジトリに存在することを確認する
3. 対象ブランチに `main` を入力し、Step.1.1 と Step.1.2 のみチェックして Issue を作成する
4. Actions タブで `auto-batch-design.yml` の Bootstrap ジョブが起動したことを確認する
...
7. step-1.1 の Issue を close し、`auto-batch-design.yml` の状態遷移ジョブが起動することを確認する
