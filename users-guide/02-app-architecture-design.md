...
```mermaid
flowchart TD
  s1(["Arch-ApplicationAnalytics [1]"])
  s2(["Arch-ArchitectureCandidateAnalyzer [2]"])
  s1 -->|"docs/catalog/app-catalog.md"| s2
  s1 -. "knowledge/ 参照" .-> kn
  s2 -. "knowledge/ 参照" .-> kn
  kn[/"knowledge/"/]
...
![AAS: Arch-ApplicationAnalytics → Arch-ArchitectureCandidateAnalyzer の2ステップチェーン（並列0箇所含む）](./images/chain-aas.svg)
...
|---------|---------|-------------|------|------|------|
| step-1 | Step.1 アプリケーションリストの作成 | `Arch-ApplicationAnalytics` | docs/catalog/use-case-catalog.md | docs/catalog/app-catalog.md | なし |
| step-2 | Step.2 ソフトウェアアーキテクチャの推薦 | `Arch-ArchitectureCandidateAnalyzer` | docs/catalog/app-catalog.md, docs/architectural-requirements-app-xx.md（存在するもののみ） | docs/catalog/app-arch-catalog.md | step-1 |
