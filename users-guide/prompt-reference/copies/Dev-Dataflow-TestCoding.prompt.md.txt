...
- 生成する pytest テストは **ローカル端末 / CI で `pytest` が実行可能**であること。
...
- `tdd-test-report.md` の `Expected Outcome` には、RED フェーズとして `pytest` がローカルで実行され、プロダクション実装未完了により失敗することを明記する。
...
- 依存定義ファイル（`requirements.txt` / `pyproject.toml` 等）— 既存があれば更新、なければ新規作成
...
- 既存の `src/test/dataflow/` 配下にテストがあれば、構成・フレームワーク・モッキングライブラリ・命名規則の"型"を踏襲する。
- Python のバージョンと依存定義の形式は既存コード/設定から確定する。見つからなければ Questions に記載する。
- テストダブルの既定ライブラリ: `unittest.mock`（または `pytest-mock`。既存で異なるものが使われている場合はそれを踏襲する）。
- 分散処理を PySpark で実装するジョブでは、ローカル実行可能な `SparkSession`（local mode）をフィクスチャとして用い、実クラスタへ接続しない。
...
- `pytest --collect-only` でテストが収集される（import エラーがない）ことを確認する。
- `pytest` でテストが **失敗する** ことを確認する（RED 状態の検証）。
- import エラーが出る場合は最小限のインターフェース/スタブを追加して解消する。
...
- `src/test/dataflow/{jobId}-{jobNameSlug}.Tests/` 配下にテストが存在し、`pytest --collect-only` が成功する。
