...
- `work-artifacts-layout`: 出力先は §0 モードディスパッチ表と実行中 Step の Body テンプレートに従う
## 1) 入力（置換必須）
> `{...}` が残っている場合は実行しない。
...
## 4) 成果物保存
- 出力先は §0 モードディスパッチ表と実行中 Step の Body テンプレートの `## 出力` セクションに従う（Skill work-artifacts-layout §4.1 準拠: delete → create）。中間成果物は `{WORK}` 配下に保存する。
...
- artifacts: §0 モードディスパッチ表の Step 別出力先に従う
```
