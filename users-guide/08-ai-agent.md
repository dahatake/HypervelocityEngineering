...
- 通常時：作成した `docs/agent/agent-application-definition.md` の内容（Markdown全文）をそのまま出力し、同内容をファイルにも保存する。
...
### Issue Template / Workflow / Custom Agent 対応（実装準拠）

| 区分 | AAG（設計） | AAGD（実装/Deploy） |
|---|---|---|
| Issue Template | `.github/ISSUE_TEMPLATE/ai-agent-design.yml` | `.github/ISSUE_TEMPLATE/ai-agent-dev.yml` |
...
|---|---|---|
| AAG | 1 | `Arch-AIAgentDesign-Step1` |
| AAG | 2 | `Arch-AIAgentDesign-Step2` |
| AAG | 3 | `Arch-AIAgentDesign-Step3` |
| AAGD | 1 | `Arch-AIAgentDesign-Step1` |
| AAGD | 2.1 | `Arch-TDD-TestSpec` |
...
各 Custom Agent の入出力ファイルを示します（`hve/workflow_registry.py` の AAG / AAGD 定義に準拠）。

![AAG: AI Agent 設計 データフロー](./images/orchestration-task-data-flow-aag.svg)

![AAGD: AI Agent 実装・デプロイ データフロー](./images/orchestration-task-data-flow-aagd.svg)
...
* `docs/agent/agent-architecture.md`（Agent Catalog + Components + 図を統合）
...
# Inputs（一次情報）
- 入力ファイル: docs/agent/agent-application-definition.md
...
Step 1〜3 で完成した設計書（アプリケーション定義・アーキテクチャ・詳細設計）を入力として、Microsoft Foundry（Azure AI Foundry Agent Service）を使用して AI Agent を実装する。

## 前提

- **Azure AI Foundry リソース（プロジェクト）が作成済みであること**（ASDW Step.2.2/2.3 の `docs/azure/azure-services-additional.md` にプロジェクト名・エンドポイント URL・リソースグループが記録されていること）
- Azure AI Foundry SDK がインストール済みであること
...
- SDK リファレンス: https://learn.microsoft.com/en-us/azure/foundry/how-to/develop/sdk-overview

> [!IMPORTANT]
> チュートリアルのサンプルコードをそのままコピーして使用しないこと。
> ユーザーが指定したプログラミング言語で、最新の Azure AI Foundry SDK を使用して実装すること。
...
| Step.2.7TC | `test/agent/{AgentName}.Tests/`（TDD テストコード — RED 状態） |
...
ASDW 自動実行ワークフロー外で手動実行する場合は、以下のプロンプトを参考にしてください。

### Step.2.7T — AI Agent テスト仕様書の手動生成

```text
# タスク
テスト戦略書・Agent 詳細設計書を根拠に、Agent 用 TDD テスト仕様書を生成する。
生成するテストカテゴリ:
1. Agent I/O 契約テスト（入力→期待出力の検証）
2. Tool モック統合テスト（Tool 呼び出しのパラメータ・戻り値・エラー時の検証）
3. Guardrails テスト（禁止操作・PII マスキング等の検証）
4. 状態遷移テスト（正常フロー・例外フロー）
5. プロンプト回帰テスト（System Prompt 変更後の動作一貫性）
...
- `docs/agent/agent-detail-*.md`
- `docs/ai-agent-catalog.md`
- `test/agent/{AgentName}.Tests/`（TDD テストコード — RED 状態）
- `docs/test-specs/{agentId}-test-spec.md`
- `docs/catalog/service-catalog-matrix.md`
