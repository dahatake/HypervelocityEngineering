...
### 3.4 PR / Issue を参照する場合の制約

Pull Request や Issue（障害記録を含む）は `markdown-query` / `code-query` の索引対象ではありません。いずれもワークツリー上のファイルだけを索引する設計のためです。Agent セッションから参照するには **`.github/.mcp.json` への宣言**が必要です。

GitHub Copilot CLI 側へプラグインとして登録しただけでは Step 実行セッションへは届きません。HVE は宣言外の MCP サーバを自動探索で取り込まないからです（本頃 §3 の `enable_config_discovery=False`）。

宣言する場合は **参照系ツールだけを列挙**してください。`tools: ["*"]` は使えません。作成・更新・削除・マージ・クローズ・push 等の状態変更ツールを含めると CI の契約テストが失敗します。
