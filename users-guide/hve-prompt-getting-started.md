...
- 複数 Workflow のうち 1 つが失敗した → 後続の Workflow は開始しません
...
`--expected-sha256` が一致しない場合、HVE は子プロセスを 1 つも起動せずに停止します。
...
| `計画が承認時と一致しません（stale）` | 依頼内容 / GUI 設定 / HEAD が変わった | Copilot に「計画を作り直してください」と伝える |
