...
アプリケーションは `docs/app-list.md`（App Selection の成果物）に定義されています。
...
以下のドキュメントが存在していることを確認してください（設計ワークフロー App Design（AAD）で作成済みであること）:
...
2. テンプレート **"App Dev Microservice"** を選択
3. 以下を入力:
...
1. `.github/ISSUE_TEMPLATE/app-dev-microservice.yml` がリポジトリに存在することを確認する
2. Issues タブ → New Issue → **App Dev Microservice** テンプレートが表示されることを確認する
...
5. Issues タブ → New Issue → **App Dev Microservice** テンプレートを選択する
