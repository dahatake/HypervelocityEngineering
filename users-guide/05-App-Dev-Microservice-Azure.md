...
ASDW（App Dev Microservice）は、ASD（Auto Software Design）ワークフローで完成した設計ドキュメントをもとに、
Issue Form から親 Issue を作成するだけで、Step.1〜Step.4 の Web アプリ実装タスクが
Sub-issue として自動生成され、Copilot が依存関係に従って順次・並列実行するワークフローです。

**本ワークフローは 1 度の実行で 1 つのアプリケーション (APP-ID) のみを対象とします。**
アプリケーションは `docs/app-list.md`（Auto App Selection Workflow の成果物）に定義されています。
...
- AI Agent 構成設計
- AI Agent 実装・デプロイ（Microsoft Foundry — Azure AI Foundry Agent Service）
...
以下のドキュメントが存在していることを確認してください（設計ワークフロー Auto App Design（AAD）で作成済みであること）:

| ファイル | 説明 |
|--------|------|
| `docs/domain-analytics.md` | ドメイン分析書（ASD Step.1.1 の成果物） |
| `docs/service-list.md` | マイクロサービス一覧（ASD Step.1.2 の成果物） |
| `docs/data-model.md` | データモデル（ASD Step.2 の成果物） |
| `docs/screen-list.md` | 画面一覧（ASD Step.3 の成果物） |
| `docs/service-catalog.md` | サービスカタログ（ASD Step.4 の成果物） |
| `docs/usecase-list.md` | ユースケース一覧 |
| `docs/screen/` | 画面定義書（ASD Step.5.1 の成果物） |
| `docs/services/` | マイクロサービス定義書（ASD Step.5.2 の成果物） |
| `docs/test-strategy.md` | テスト戦略書（必須） |
| `docs/test-specs/{serviceId}-test-spec.md` | サービス別テスト仕様書（AAD Step.7.3 の成果物） |
| `docs/test-specs/{screenId}-test-spec.md` | 画面別テスト仕様書（AAD Step.7.3 の成果物） |
| `src/data/sample-data.json` | サンプルデータ（ASD Step.2 の成果物） |
| `users-guide/08-AIAgent.md` | AI Agent 設計ガイドライン（Step.2.6 で使用） |
| `docs/app-list.md` | アプリケーション一覧（存在しない場合はアプリケーション名を直接入力可） |

### 必須シークレット

- `COPILOT_PAT` — Copilot coding agent のアサインに使用
- `AZURE_STATIC_WEB_APPS_API_TOKEN` — Step.3.2 の SWA デプロイで使用
- `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID` — Step.2.8 の AI Agent デプロイ（OIDC 認証）で使用
...
```
[Issue作成]
    │ label: auto-app-dev-microservice
    ▼
[Bootstrap Workflow]
    ├─ ラベル自動作成
    ├─ Step.1〜Step.4 の Sub-issue 一括生成
    ├─ Step.1.1: asdw:ready + asdw:running + Copilot assign（最初の leaf Step）
    └─ Step.1.2〜Step.4.2: ラベルなし（依存先完了待ち）

[Step Issue asdw:done ラベル付与または close]
    └─ [auto-app-dev-microservice-azure.yml 状態遷移] → 依存関係チェック → 次 Step を asdw:ready + asdw:running + Copilot assign

[全 Step 完了] → 親 Issue に完了通知コメント
```
...
- GitHub Copilot Coding Agent / GitHub Copilot for Azure
...
|-----------|---------|--------------|------|------|------|
| step-1.1  | Azure データストア選定 | `Dev-Microservice-Azure-DataDesign` | `docs/data-model.md`, `docs/service-list.md`, `docs/domain-analytics.md` | `docs/azure/AzureServices-data.md` | なし |
| step-1.2  | Azure データサービス Deploy | `Dev-Microservice-Azure-DataDeploy` | `docs/azure/AzureServices-data.md`, `docs/service-catalog.md`, `src/data/sample-data.json` | `infra/azure/create-azure-data-resources*.sh`, `src/data/azure/data-registration-script.sh`, `infra/azure/verify-data-resources.sh`, `docs/test-specs/deploy-step1-data-test-spec.md` | step-1.1 |

#### Step.2: マイクロサービス作成

| Step ID   | タイトル | Custom Agent | 入力 | 出力 | 依存 |
|-----------|---------|--------------|------|------|------|
| step-2.1  | Azure コンピュート選定 | `Dev-Microservice-Azure-ComputeDesign` | `docs/service-list.md`, `docs/usecase-list.md`, `docs/data-model.md`, `docs/service-catalog.md` | `docs/azure/AzureServices-services.md` | Step.1 完了 |
| step-2.2  | 追加 Azure サービス選定 | `Dev-Microservice-Azure-AddServiceDesign` | `docs/usecase-list.md`, `docs/service-list.md`, `docs/services/`, `docs/azure/AzureServices-services.md` | `docs/azure/AzureServices-services-additional.md` | step-2.1 |
| step-2.3  | 追加 Azure サービス Deploy | `Dev-Microservice-Azure-AddServiceDeploy` | `docs/azure/AzureServices-services-additional.md` | `infra/azure/create-azure-additional-resources*`, `infra/azure/verify-additional-resources.sh`, `docs/test-specs/deploy-step2-additional-test-spec.md` | step-2.2 |
...
| step-2.5  | Azure Compute Deploy | `Dev-Microservice-Azure-ComputeDeploy-AzureFunctions` | `docs/service-list.md`, `docs/service-catalog.md`, `src/api/` | `.github/workflows/`, `infra/azure/create-azure-api-resources-prep.sh`, `infra/azure/verify-api-resources.sh`, `docs/test-specs/deploy-step2-compute-test-spec.md` | step-2.4 |
| step-2.6  | AI Agent 構成設計 | `Arch-AIAgentDesign` | ユースケース記述, `docs/service-catalog.md`, `docs/azure/` | `docs/AI-Agents-list.md`, `docs/agent/` | step-2.5 |
| step-2.7T | AI Agent テスト仕様書 (TDD RED) | `Arch-TDD-TestSpec` | `docs/test-strategy.md`, `docs/AI-Agents-list.md`, `docs/agent/agent-detail-*.md`, `docs/service-catalog.md`, `docs/data-model.md` | `docs/test-specs/{agentId}-test-spec.md` | step-2.6 |
| step-2.7TC| AI Agent テストコード生成 (TDD RED) | `Dev-Microservice-Azure-AgentTestCoding` | `docs/test-specs/{agentId}-test-spec.md`, `docs/agent/agent-detail-*.md`, `docs/service-catalog.md` | `test/agent/{AgentName}.Tests/` | step-2.7T |
| step-2.7  | AI Agent 実装 (TDD GREEN) | `Dev-Microservice-Azure-AgentCoding` | `docs/agent/agent-detail-*.md`, `docs/AI-Agents-list.md`, `test/agent/{AgentName}.Tests/`, `docs/test-specs/{agentId}-test-spec.md`, `docs/service-catalog.md`, `docs/azure/AzureServices-services-additional.md` | `src/agent/{AgentID}-{AgentName}/` | step-2.7TC |
| step-2.8  | AI Agent Deploy | `Dev-Microservice-Azure-AgentDeploy` | `src/agent/{AgentID}-{AgentName}/`, `docs/AI-Agents-list.md`, `docs/azure/AzureServices-services-additional.md`, リソースグループ名 | `infra/azure/create-azure-agent-resources-prep.sh`, `infra/azure/create-azure-agent-resources.sh`, `infra/azure/verify-agent-resources.sh`, `.github/workflows/deploy-agent-*.yml`, `docs/test-specs/deploy-step2-agent-test-spec.md`, `docs/azure/service-catalog.md`（Agent エンドポイント追記） | step-2.7 |
...
| step-3.1  | UI 実装 (TDD GREEN) | `Dev-Microservice-Azure-UICoding` | `docs/screen/`, `docs/screen-list.md`, `docs/service-catalog.md`, `test/ui/` | `src/app/` | step-3.0TC |
...
| Step ID   | タイトル | Custom Agent | 入力 | 出力 | 依存 |
|-----------|---------|--------------|------|------|------|
| step-4.1  | WAF アーキテクチャレビュー | `QA-AzureArchitectureReview` | `docs/service-catalog.md`, `docs/azure/`, リソースグループ名 | `docs/azure/Azure-ArchitectureReview-Report.md` | Step.3 完了（step-4.2 と並列） |
| step-4.2  | 整合性チェック | `QA-AzureDependencyReview` | `docs/service-catalog.md`, `docs/azure/`, `src/`, `infra/`, `.github/workflows/` | `docs/azure/DependencyReview-Report.md` | Step.3 完了（step-4.1 と並列） |

### 成果物一覧

| Step | 成果物 |
|------|--------|
| Step.1.1 | `docs/azure/AzureServices-data.md` |
...
| Step.2.1 | `docs/azure/AzureServices-services.md` |
| Step.2.2 | `docs/azure/AzureServices-services-additional.md` |
...
| Step.2.6 | `docs/AI-Agents-list.md`, `docs/agent/` |
...
| Step.4.1 | `docs/azure/Azure-ArchitectureReview-Report.md` |
| Step.4.2 | `docs/azure/DependencyReview-Report.md` |
...
- 対象アプリケーション: `{APP-ID}`（`docs/app-list.md` を参照し、対象 APP-ID に紐づくエンティティのみを処理対象とする）
...
> [!NOTE]
> AI Agent 設計の詳細ガイドは [08-AIAgent.md](./08-AIAgent.md) を参照してください。

```text
# タスク
ユースケース記述を入力として、AI Agent のアプリケーション定義・粒度設計・詳細設計を一貫して実施し、docs/AI-Agents-list.md に Agent 一覧を出力する。

# 入力
- ユースケース記述: `docs/usecase-list.md` または `docs/usecase/{USECASE_ID}/`
- `docs/service-catalog.md`
- `docs/azure/`
...
3. `docs/service-catalog.md`（WebアプリURL追記）
4. `.github/workflows/azure-static-web-apps-*.yml`（`Azure/static-web-apps-deploy@v1` 使用）
```
...
2. テンプレート **"App Dev Microservice Workflow"** を選択
3. 以下を入力:
   - **対象アプリケーション (APP-ID)**: `docs/app-list.md` からアプリケーション ID を入力 (例: `APP-01`)。`docs/app-list.md` が存在しない場合はアプリケーション名を入力 (例: `royalty-service`)
   - **対象ブランチ**: コード/ドキュメントをコミットするブランチ名 (例: `main`)
   - **リソースグループ名**: Azure のリソースグループ名 (例: `rg-royalty-service-dev`)
   - **ユースケースID** (任意): AI Agent 設計（Step.2.6）で使用するユースケースID (例: `UC-001`)
...
詳細（Azure Resource ID、コンテナ一覧、SKU）は [`docs/azure/service-catalog.md`](../docs/azure/service-catalog.md) を参照。
...
`work` フォルダーに作成されたPromptファイルを GitHub Copilot Coding Agent の Sub Issue として実行します。
...
1. `.github/ISSUE_TEMPLATE/auto-app-dev-microservice.yml` がリポジトリに存在することを確認する
2. Issues タブ → New Issue → **App Dev Microservice Workflow** テンプレートが表示されることを確認する
...
2. `.github/workflows/auto-app-dev-microservice-azure.yml` がリポジトリに存在することを確認する
3. ASD ワークフロー完了後の前提ドキュメント（`docs/domain-analytics.md` 等）が存在することを確認する
4. `users-guide/08-AIAgent.md` がリポジトリに存在することを確認する（Step.2.6 AI Agent 設計で使用）
5. Issues タブ → New Issue → **App Dev Microservice Workflow** テンプレートを選択する
6. ブランチ `main`、リソースグループ名、Step.1 のみチェックして Issue を作成する
7. Actions タブで `ASDW Orchestrator` の Bootstrap ジョブが起動したことを確認する
8. Bootstrap 完了後、Step.1.1 の Issue が `asdw:running` ラベル付きで作成されたことを確認する（他の Step はラベルなし）
9. 親 Issue にサマリコメントと Step Issue 一覧が投稿されたことを確認する
10. Step.1.1 の Issue を close し、`auto-app-dev-microservice-azure.yml` の状態遷移ジョブが起動することを確認する
11. Step.1.2 に `asdw:ready` + `asdw:running` が付与され Copilot が assign されることを確認する
12. Step.1.2 を close し、Step.2.1 が起動することを確認する（Step.1 コンテナに `asdw:done` 付与）
13. Step.1〜4 すべてを選択して Issue を作成し直し、全 Step Issue が生成されることを確認する
14. Step.2.6 を close して Step.2.7T が起動することを確認する（Step.2 コンテナはまだ完了しない）
15. Step.2.7T → Step.2.7TC → Step.2.7 → Step.2.8 の順で進むことを確認する
16. Step.2.8 を close して Step.2 コンテナに `asdw:done` が付与され、Step.3.0TC が起動することを確認する
17. Step.3.0TC → Step.3.1 → Step.3.2 の順で進むことを確認する
18. Step.3.2 を close して Step.4.1 と Step.4.2 が**並列で**開始されることを確認する
19. Step.4.1 を close して Root Issue への完了通知がまだ届かないことを確認する（Step.4.2 が未完了のため）
20. Step.4.2 を close して Root Issue に完了通知コメントが投稿されることを確認する（AND依存解消）
