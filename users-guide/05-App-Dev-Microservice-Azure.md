...
以下のドキュメントが存在していることを確認してください（設計ワークフロー App Detail Design（AAD）で作成済みであること）:

| ファイル | 説明 |
|--------|------|
| `docs/catalog/domain-analytics.md` | ドメイン分析書（ASD Step.1.1 の成果物） |
| `docs/catalog/service-catalog.md` | マイクロサービス一覧（ASD Step.1.2 の成果物） |
| `docs/catalog/data-model.md` | データモデル（ASD Step.2 の成果物） |
| `docs/catalog/screen-catalog.md` | 画面一覧（ASD Step.3 の成果物） |
| `docs/catalog/service-catalog-matrix.md` | サービスカタログ（ASD Step.4 の成果物） |
...
| `users-guide/08-ai-agent.md` | AI Agent 設計ガイドライン（Step.2.6 で使用） |
...
flowchart TD
  subgraph L1["レイヤー 1"]
    s1_1["Dev-Microservice-Azure-DataDesign [1.1]"]
    s1_2["Dev-Microservice-Azure-DataDeploy [1.2]"]
  end
  subgraph L2["レイヤー 2"]
    s2_1["Dev-Microservice-Azure-ComputeDesign [2.1]"]
    s2_2["Dev-Microservice-Azure-AddServiceDesign [2.2]"]
    s2_3["Dev-Microservice-Azure-AddServiceDeploy [2.3]"]
    s2_3T(["Arch-TDD-TestSpec [2.3T]"])
    s2_3TC["Dev-Microservice-Azure-ServiceTestCoding [2.3TC]"]
    s2_4["Dev-Microservice-Azure-ServiceCoding-AzureFunctions [2.4]"]
    s2_5["Dev-Microservice-Azure-ComputeDeploy-AzureFunctions [2.5]"]
    s2_6(["Arch-AIAgentDesign [2.6]"])
    s2_7T(["Arch-TDD-TestSpec [2.7T]"])
    s2_7TC["Dev-Microservice-Azure-AgentTestCoding [2.7TC]"]
    s2_7["Dev-Microservice-Azure-AgentCoding [2.7]"]
    s2_8["Dev-Microservice-Azure-AgentDeploy [2.8]"]
  end
  subgraph L3["レイヤー 3"]
    s3_0T(["Arch-TDD-TestSpec [3.0T]"])
    s3_0TC["Dev-Microservice-Azure-UITestCoding [3.0TC]"]
    s3_1["Dev-Microservice-Azure-UICoding [3.1]"]
    s3_2["Dev-Microservice-Azure-UIDeploy-AzureStaticWebApps [3.2]"]
  end
  subgraph L4["レイヤー 4"]
    s4_1{{"QA-AzureArchitectureReview [4.1]"}}
    s4_2{{"QA-AzureDependencyReview [4.2]"}}
  end
  s1_1 -->|"docs/azure/azure-services-data.md"| s1_2
  s1_2 -->|"{WORK}"| s2_1
  s2_1 -->|"{WORK}artifacts/"| s2_2
  s2_2 -->|"docs/azure/azure-services-additional.md"| s2_3
  s2_2 -. "docs/azure/azure-services-additional.md（代替依存）" .-> s2_3
  s2_3 -->|"infra/azure/create-azure-additional-resources-prep.sh"| s2_3T
  s2_3 -. "infra/azure/create-azure-additional-resources-prep.sh（代替依存）" .-> s2_3T
  s2_3T -->|"{WORK}"| s2_3TC
  s2_3T -. "{WORK}（代替依存）" .-> s2_3TC
  s2_3TC -->|"{WORK}"| s2_4
  s2_3TC -. "{WORK}（代替依存）" .-> s2_4
  s2_4 -->|"{WORK}"| s2_5
  s2_5 -->|"{WORK}"| s2_6
  s2_5 -. "{WORK}（代替依存）" .-> s2_6
  s2_6 -->|"docs/agent/agent-application-definition.md"| s2_7T
  s2_6 -. "docs/agent/agent-application-definition.md（代替依存）" .-> s2_7T
  s2_7T -->|"{WORK}"| s2_7TC
  s2_7T -. "{WORK}（代替依存）" .-> s2_7TC
  s2_7TC -->|"{WORK}"| s2_7
  s2_7TC -. "{WORK}（代替依存）" .-> s2_7
  s2_7 -->|"{WORK}"| s2_8
  s2_7 -. "{WORK}（代替依存）" .-> s2_8
  s2_8 -->|"{WORK}"| s3_0T
  s2_8 -. "{WORK}（代替依存）" .-> s3_0T
  s3_0T -->|"{WORK}"| s3_0TC
  s3_0T -. "{WORK}（代替依存）" .-> s3_0TC
  s3_0TC -->|"{WORK}"| s3_1
  s3_0TC -. "{WORK}（代替依存）" .-> s3_1
  s3_1 -->|"{WORK}"| s3_2
  s3_2 -->|"{WORK}"| s4_1
  s3_2 -->|"{WORK}"| s4_2
  ...
  s2_3T -. "knowledge/ 参照" .-> kn
  s2_3TC -. "knowledge/ 参照" .-> kn
  s2_4 -. "knowledge/ 参照" .-> kn
  s2_5 -. "knowledge/ 参照" .-> kn
  s2_6 -. "knowledge/ 参照" .-> kn
  s2_7T -. "knowledge/ 参照" .-> kn
  s2_7TC -. "knowledge/ 参照" .-> kn
  s2_7 -. "knowledge/ 参照" .-> kn
  s2_8 -. "knowledge/ 参照" .-> kn
  s3_0T -. "knowledge/ 参照" .-> kn
  s3_0TC -. "knowledge/ 参照" .-> kn
  ...
  class s4_2 qa;
  class kn file;
  style s2_3T stroke-width:3px
  style s2_7T stroke-width:3px
  style s3_0T stroke-width:3px
  subgraph LEG["凡例"]
    la(["Arch-*（青）: 設計"]):::arch
    ld["Dev-*（緑）: 実装"]:::dev
    lc{"Doc-*（紫）: 文書生成"}:::doc
...
3. ASD ワークフロー完了後の前提ドキュメント（`docs/catalog/domain-analytics.md` 等）が存在することを確認する
4. `users-guide/08-ai-agent.md` がリポジトリに存在することを確認する（Step.2.6 AI Agent 設計で使用）
