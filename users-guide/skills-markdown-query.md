...
| ③ Indexing | `indexer.py`, `strategies.py`, `strategies_semantic.py`, `embeddings.py`, `sentence_splitter.py`, `contextualizer.py`, `tokenize.py` | Markdown を Chunking Strategy ごとに分割し、永続化用 row を生成 | `cmd_index` / `cmd_watch` |
...
| ⑥ Storage | `store.py` + `.mdq/index-<lang>-<strategy>.sqlite` | スキーマ管理 (SCHEMA v5) + マイグレーション + CRUD | `open_store()` 経由 |
...
| R4 | 概念語 (`概要` / `とは` / `what` 等) | heading | `concept_overview` |
...
`semantic_paragraph → heading_recursive → heading → fixed_window`
...
`mdq.strategies.ALL_STRATEGIES` に 4 種の戦略が定義されている。同一 Markdown 入力に対する境界の違いは以下:
...
| 同 `chunks.text_raw` 列 (SCHEMA v5) | contextualize ON 時の原文保存 | semantic_paragraph 索引時のみ | 索引再構築時 | `chunks.body` の 0.7 倍程度 |
| 同 `chunks.chunk_embedding` 列 (SCHEMA v5) | float32 埋め込みベクトル (768 次元 BAAI/bge-m3) | semantic_paragraph + `--late-chunking` 索引時のみ | 索引再構築時 | チャンク数 × 3 KB |
...
- セットアップ手順: [tools/skills/markdown_query/SETUP.md](../tools/skills/markdown_query/SETUP.md)
...
- Chunking Strategy:
  ...
  - `semantic_paragraph`: 見出しを **hard boundary** とし、各見出し chunk 内を
    ...
    fastembed + nltk + numpy を追加し、`BAAI/bge-m3` を既定 model として使用
...
- SCHEMA v5: `text_raw`（原文。contextualize ON の場合のみ非 NULL）と
  `chunk_embedding`（float32 BLOB。`--late-chunking` 有効時のみ非 NULL）列が追加。
  どちらも ADD COLUMN マイグレーションで既存 DB は破壊されない。
