...
1. ワークフロー選択メニューで **`1) Auto Requirement Definition (ard — 4 グループ)`** を選びます。
2. モデルと実行モードを選びます（クイック全自動 / カスタム全自動 / 手動）。
3. **実行するグループを選択** します（既定 ON: `[2]` / `[3]` / `[4]`、`[1]` は既定 OFF）:
...
`docs/business-requirement.md`（または `docs/company-business-requirement.md`）の **戦略的記述**（To-Be ビジョン / 戦略的方向性 / 戦略的アクションプラン / 主要 KPI に相当する記述）を根拠に、後続のユースケース作成・アプリケーション選定・実装が **継続的に「戦略達成度」を測定可能** にするための KPI/OKR、計測データ定義、目的志向のデータ収集設計を作成します。

> **用語: 目的志向のデータ収集設計** — KPI/OKR の達成判定に必要なデータを逆算し、最小限のメトリクス・イベント・属性のみを収集する設計手法。「データを集めてから何ができるか考える」ではなく「達成判定のために何を集めるか」をスタート地点とする。
...
- `docs/recommended-kpi-okr.md` — 推奨 KPI/OKR 定義書（必須 7 セクション構成）
...
```bash
python -m hve orchestrate --workflow ard --include-kpi-okr \
  --company-name "株式会社サンプル" \
  --target-business "ロイヤルティプログラム事業"
```
...
```bash
python -m hve orchestrate --workflow ard --steps 2,3,4 \
  --company-name "株式会社サンプル" \
  --target-business "ロイヤルティプログラム事業"
```

`--steps` に `3` を含めると `--include-kpi-okr` と同等の効果が得られる。

### 成果物の構成

`docs/recommended-kpi-okr.md` は以下の必須セクションを含みます:

1. Executive Summary
2. 戦略目標マッピング（`ST-*` ID）
3. KPI 定義（SMART・`KPI-*` ID）
4. OKR 定義（`OKR-*` Objective + `KR-*-*` Key Results 3-5 個）
5. 計測データ定義（5.1 定量 / 5.2 定性・`DAT-*` ID）
6. データ収集設計（目的志向・実装インターフェース仕様：イベント名・属性スキーマ・計測実装手段）
7. ID 命名規約と後続工程引き継ぎ

各 KPI/OKR には **信頼度区分**（資料上確認できる事実 / 外部情報補足 / 合理的仮説 / 追加確認必要論点）が必須付与されます（捾造防止）。
...
`docs/recommended-kpi-okr.md` が存在しない（Step 3 を実行しない）場合、後続ステップは何も参照せず通常通り動作します。`app-catalog.md` の「対応 KPI/OKR」列は空欄のままになります。
...
![Step.5.2 全体フロー図](./images/business-requirement-qa-flow.svg)
...
### Step.5.5 KnowledgeManager による文書クラス分類
...
出典: `.github/prompts/KnowledgeManager.prompt.md`
...
2. **Prompt を選択**: `.github/prompts/` から該当する Prompt を選択して Issue を作成・実行
