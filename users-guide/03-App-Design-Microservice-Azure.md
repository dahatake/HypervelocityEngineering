...
> [!NOTE]
> フェーズ1（アプリケーション選定: Step.1〜Step.2）は [02-AppSelection.md](./02-AppSelection.md) を参照してください。
> **Step.1 以降** は **Microservice + Polyglot Persistence アーキテクチャ** を前提としたステップです。将来的に他のアーキテクチャ（Batch, IoT 等）向けの設計 Step が追加される予定です。
...
> - `docs/app-list.md` — Auto App Selection Workflow (Step.1) の成果物
> - `docs/usecase-list.md` — ユースケース定義（Step.1.1/1.2 の必須入力）

フェーズ1（アプリケーション選定: Step.1〜Step.2）は **Auto App Selection Workflow** を使用してください。

### 前提条件

- `docs/app-list.md` が存在すること（Auto App Selection Workflow の成果物）
- `docs/usecase-list.md` が存在すること
...
> 💡 **複数回実行**: Auto App Selection Workflow の成果物（`docs/app-list.md`）が存在していれば、Auto App Design Workflow は何度でも実行できます。別ブランチを指定することで、異なる設計方針を並行比較することも可能です。

---

## ツール

GitHub Copilot Coding Agent を使用して、コードの作成や、Azure のリソースの作成などを行います。
...
| step-1.1 | Step.1.1 ドメイン分析 | `Arch-Microservice-DomainAnalytics` | docs/usecase-list.md | docs/domain-analytics.md | フェーズ1（AAS）完了後 |
| step-1.2 | Step.1.2 サービス一覧抽出 | `Arch-Microservice-ServiceIdentify` | docs/usecase-list.md, docs/domain-analytics.md, docs/app-list.md | docs/service-list.md | step-1.1 |
| step-2 | Step.2 データモデル | `Arch-DataModeling` | docs/domain-analytics.md, docs/service-list.md, docs/app-list.md | docs/data-model.md, src/data/sample-data.json | step-1.2 |
| step-3 | Step.3 データカタログ | `Arch-DataCatalog` | docs/data-model.md, docs/domain-analytics.md, docs/app-list.md | docs/data-catalog.md | step-2 |
| step-4 | Step.4 画面一覧 | `Arch-UI-List` | docs/domain-analytics.md, docs/service-list.md, docs/data-model.md, docs/app-list.md | docs/screen-list.md | step-3 |
| step-5 | Step.5 サービスカタログ | `Arch-Microservice-ServiceCatalog` | docs/service-list.md, docs/data-model.md, docs/screen-list.md, docs/domain-analytics.md, docs/app-list.md | docs/service-catalog.md | step-4 |
| step-6 | Step.6 テスト戦略書 | `Arch-TDD-TestStrategy` | docs/service-catalog.md, docs/data-model.md, docs/domain-analytics.md, docs/service-list.md, docs/app-list.md, docs/screen-list.md, docs/data-catalog.md | docs/test-strategy.md | step-5 |
| step-7 | Step.7 コンテナ | — | — | — | step-6（またはスキップ時は step-5） |
| step-7.1 | Step.7.1 画面定義書 | `Arch-UI-Detail` | docs/screen-list.md, docs/app-list.md, docs/test-strategy.md（存在する場合） | docs/screen/<画面-ID>-<画面名>-description.md | step-6（またはスキップ時は step-5） |
| step-7.2 | Step.7.2 マイクロサービス定義書 | `Arch-Microservice-ServiceDetail` | docs/service-catalog.md, docs/app-list.md, docs/test-strategy.md（存在する場合） | docs/services/{serviceId}-{serviceNameSlug}-description.md | step-6（またはスキップ時は step-5） |
| step-7.3 | Step.7.3 TDDテスト仕様書 | `Arch-TDD-TestSpec` | docs/test-strategy.md（存在する場合）, docs/service-catalog.md, docs/data-model.md, docs/domain-analytics.md, docs/app-list.md, 画面定義書, サービス定義書 | `docs/test-specs/{serviceId}-test-spec.md`（サービスごとに1ファイル）+ `docs/test-specs/{screenId}-test-spec.md`（画面ごとに1ファイル）。Dev ワークフロー ASDW Step.2.3TC / Step.3.0TC の前提条件 | step-6（またはスキップ時は step-5）+ step-7.1 + step-7.2 |
| step-8 | Step.8 AI Agent 設計コンテナ | — | — | — | step-7.3 |
| step-8.1 | Step.8.1 AI Agent アプリケーション定義 | `Arch-AIAgentDesign` | docs/usecase-list.md, docs/service-catalog.md, docs/domain-analytics.md, docs/data-model.md, docs/service-list.md, docs/services/SVC-*.md, docs/app-list.md, users-guide/08-AIAgent.md | docs/agent/agent-application-definition.md | step-7.3 |
| step-8.2 | Step.8.2 AI Agent 粒度設計 | `Arch-AIAgentDesign` | Step.8.1 の成果物 + 上記入力 | docs/agent/agent-architecture.md | step-8.1 |
| step-8.3 | Step.8.3 AI Agent 詳細設計 | `Arch-AIAgentDesign` | Step.8.2 の成果物 + Agent Catalog | docs/agent/agent-detail-<Agent-ID>-<Agent名>.md, docs/AI-Agents-list.md | step-8.2 |
...
- `docs/domain-analytics.md`
- `docs/app-list.md`（アプリケーション一覧 — 各サービス候補に APP-ID を紐付けること）
...
- `docs/domain-analytics.md`
- `docs/service-list.md`
- `docs/app-list.md`（アプリケーション一覧 — Entity Catalog の各エンティティに APP-ID を紐付けること）
...
```text
# タスク
データモデル（docs/data-model.md）とドメイン分析（docs/domain-analytics.md）を根拠に、概念エンティティ × 物理テーブル/列のマッピングを記録するデータカタログを作成する（推測禁止、出典明記）。

# 入力
- `docs/data-model.md`（必須）
- `docs/domain-analytics.md`（必須）
- `docs/app-list.md`（アプリケーション一覧 — Entity-Table Mapping と Ownership Matrix に APP-ID を紐付けること）
- `docs/service-list.md`（存在すれば読む）
- `docs/service-catalog.md`（存在すれば読む）
...
- 最優先：
  ...
  - `docs/app-list.md`（アプリケーション一覧 — 各画面に所属 APP-ID を紐付けること。1画面:1APP）
...
- `docs/screen-list.md`
- `docs/domain-analytics.md`
- `docs/app-list.md`（アプリケーション一覧 — Table A に所属APP、Table C に利用APP を記載すること）
...
- `docs/domain-analytics.md`
- `docs/service-list.md`
- `docs/app-list.md`（アプリケーション一覧 — テスト戦略書でアプリ単位のサービス分類に参照）
- `docs/data-catalog.md`（存在すれば参照）
- `docs/screen-list.md`（存在すれば参照）
...
```text
# タスク
docs/screen-list.md の全画面について、実装に使える画面定義書（UX/A11y/セキュリティ含む）を docs/screen/ に生成・更新する。
...
推奨（存在すれば読む）:
- `docs/app-list.md`（アプリケーション一覧 — 各画面定義書の「§1 概要」に所属 APP-ID を記載すること）
- `docs/domain-analytics.md`
- `docs/service-list.md`
- `docs/data-model.md`
- `docs/service-catalog.md`
...
> ⚠️ **前提条件**: Step 7.3 が完了していること（`docs/services/`, `docs/service-catalog.md`, `docs/domain-analytics.md` 等が存在すること）
...
- `docs/app-list.md`
- `users-guide/08-AIAgent.md`（設計ガイドライン Step 1）
...
- `docs/service-catalog.md`, `docs/domain-analytics.md`, `docs/data-model.md`
- `docs/app-list.md`
- `users-guide/08-AIAgent.md`（設計ガイドライン Step 2）
...
- `docs/agent/agent-application-definition.md`（Step 8.1 の成果物）
- `docs/service-catalog.md`, `docs/services/SVC-*.md`
- `docs/app-list.md`
- `users-guide/08-AIAgent.md`（設計ガイドライン Step 3）
...
2. テンプレート **"Auto App Design Workflow"** を選択
3. 以下を入力:
...
- **前提条件ファイルが不足している場合**: `docs/app-list.md` が存在しない → **Auto App Selection Workflow** の Step.1 を先に実行してください
...
2. `docs/app-list.md` が存在することを確認する（Auto App Selection Workflow の成果物）
3. `docs/usecase-list.md` が存在することを確認する
4. `.github/workflows/auto-app-design.yml` がリポジトリに存在することを確認する
5. `.github/ISSUE_TEMPLATE/auto-app-design.yml` がリポジトリに存在することを確認する
6. Issues タブ → New Issue → **Auto App Design Workflow** テンプレートを選択する
