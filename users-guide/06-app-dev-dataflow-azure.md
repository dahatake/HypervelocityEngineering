...
- [対象読者](#対象読者)
- [前提条件](#前提条件)
- [Agent チェーン図（ADFDV）](#agent-チェーン図abdv)
- [全体フロー概観](#全体フロー概観)
...
- [ADFDV ワークフローステップ（step-1.1〜step-4.*）の完了を確認する](#adfdv-ワークフローステップstep-11step-4の完了を確認する)
- [実装・デプロイへの移行（ADFDV 完了後）](#実装デプロイへの移行abdv-完了後)
- [よくある失敗例と対処法](#よくある失敗例と対処法)
- [参照ファイル一覧](#参照ファイル一覧)
- [事後 HITL フロー（リトライ上限超過後）](#事後-hitl-フローリトライ上限超過後)
- [次のステップ](#次のステップ)

---
> 初見ユーザーが、**Issue 作成 → Sub Issue 自動生成 → Custom Agent 実行 → PR マージ** の流れを起点に、ADFDV ワークフローステップ（`step-1.1`〜`step-4.*`）の完了確認から実装・デプロイ移行まで迷わず進められるよう整理したガイドです。
...
| 対象 | 前提スキル |
|---|---|
| データフロー処理設計・実装を担当するエンジニア | GitHub Issues / Actions の基本操作 |
...
バッチ実装フェーズを開始するには、**設計フェーズ**（ADFD ワークフロー）の成果物が事前に必要です。
...
![ADFDV アーキテクチャ: 入力ファイル → auto-dataflow-dev Workflow → Custom Agent チェーン → 成果物](./images/infographic-adfdv.svg)

### データフロー図（ADFDV）

以下の図は、各ステップで Custom Agent が読み書きするファイルのデータフローを示します。

![ADFDV データフロー: 各 Custom Agent の入出力ファイル](./images/orchestration-task-data-flow-adfdv.svg)
...
2. テンプレート一覧から **"Dataflow Dev"** を選択して **"Get started"** をクリック
...
| 対象データフローアプリ ID | 実装対象のジョブ ID（カンマ区切り・任意） | `BJ-001, BJ-002` |
...
- Issue を Submit すると、`auto-dataflow-dev` ラベルが自動付与されます
- Actions タブで `auto-orchestrator-dispatcher.yml`（dispatcher）と `auto-dataflow-dev-reusable.yml` が起動したことを確認してください

> ✅ 確認ポイント: `auto-dataflow-dev-reusable.yml` が表示されて実行中になること
...
2. **最初の実行対象 Step に `adfdv:ready` + `adfdv:running` ラベルが付与される**
...
|---|---|---|---|
| step-1.1 | [`Dev-Dataflow-DataServiceSelect`](../.github/agents/Dev-Dataflow-DataServiceSelect.agent.md) | `docs/dataflow/dataflow-service-catalog.md`, `docs/dataflow/dataflow-app-catalog.md` | `infra/azure/dataflow/create-batch-resources.sh`, `infra/azure/dataflow/verify-batch-resources.sh` |
| step-1.2 | [`Dev-Dataflow-DataDeploy`](../.github/agents/Dev-Dataflow-DataDeploy.agent.md) | `docs/dataflow/dataflow-service-catalog.md`, `docs/dataflow/dataflow-app-catalog.md`, `infra/azure/dataflow/create-batch-resources.sh`, `infra/azure/dataflow/verify-batch-resources.sh` | Azure データリソース実行ログ・検証結果（`work/` 配下） |
| step-2.1 | [`Dev-Dataflow-TestCoding`](../.github/agents/Dev-Dataflow-TestCoding.agent.md) | `docs/test-specs/{jobId}-test-spec.md` | `test/dataflow/{jobId}-{jobNameSlug}.Tests/` |
| step-2.2 | [`Dev-Dataflow-ServiceCoding`](../.github/agents/Dev-Dataflow-ServiceCoding.agent.md) | `docs/dataflow/apps/{jobId}-{jobNameSlug}-spec.md`, step-2.1 成果物 | `src/dataflow/` 実装コード |
| step-3 | [`Dev-Dataflow-FunctionsDeploy`](../.github/agents/Dev-Dataflow-FunctionsDeploy.agent.md) | `src/dataflow/`, `docs/dataflow/dataflow-service-catalog.md`, `docs/dataflow/dataflow-app-catalog.md` | `.github/workflows/deploy-batch-functions.yml`, `infra/azure/dataflow/README.md` |
| step-4.1 | [`QA-AzureArchitectureReview`](../.github/agents/QA-AzureArchitectureReview.agent.md) | step-3 成果物, `docs/dataflow/dataflow-service-catalog.md` | `docs/azure/azure-architecture-review-report.md` |
| step-4.2 | [`QA-AzureDependencyReview`](../.github/agents/QA-AzureDependencyReview.agent.md) | step-3 成果物, `docs/dataflow/dataflow-service-catalog.md` | `docs/azure/dependency-review-report.md` |
...
□ 成果物ファイルが正しいパスに生成されているか
  例: infra/azure/, test/dataflow/{jobId}-{jobNameSlug}.Tests/, src/dataflow/, docs/azure/
...
- ADFDV Orchestrator が次の Step の依存関係を確認する
- 依存が解消された次の Step Issue に `adfdv:ready` + `adfdv:running` が付与される
...
3. 以下の成果物が PR に反映されていることを確認する
   - step-1.1: `infra/azure/dataflow/create-batch-resources.sh`, `infra/azure/dataflow/verify-batch-resources.sh`（リソース作成・検証スクリプト）
   - step-1.2: Azure データリソースのデプロイ・検証結果（`work/` 配下ログ）
   - step-2.1: `test/dataflow/{jobId}-{jobNameSlug}.Tests/`
   - step-2.2: `src/dataflow/` 実装コード
   - step-3: `.github/workflows/deploy-batch-functions.yml`, `infra/azure/dataflow/README.md`
...
## 実装・デプロイへの移行（ADFDV 完了後）

ADFDV 完了後は、必要に応じて運用改善・追加ジョブ実装・監視強化のタスクへ進みます。継続タスクは `create-subissues` フローで分割し、実装は `Dev-Dataflow-*`、レビューは `QA-*` Agent を使って進めてください（下記「利用可能な Custom Agent（ADFDV）」参照）。
...
### 利用可能な Custom Agent（ADFDV）

| Agent 名 | ファイル | 主な用途 |
|---|---|---|
| `Dev-Dataflow-DataServiceSelect` | [`.github/agents/Dev-Dataflow-DataServiceSelect.agent.md`](../.github/agents/Dev-Dataflow-DataServiceSelect.agent.md) | Azure データサービス選定・リソース作成スクリプト生成（step-1.1） |
| `Dev-Dataflow-DataDeploy` | [`.github/agents/Dev-Dataflow-DataDeploy.agent.md`](../.github/agents/Dev-Dataflow-DataDeploy.agent.md) | Azure データリソースの実行デプロイ・検証（step-1.2） |
| `Dev-Dataflow-TestCoding` | [`.github/agents/Dev-Dataflow-TestCoding.agent.md`](../.github/agents/Dev-Dataflow-TestCoding.agent.md) | バッチ TDD RED フェーズ（テストコード生成）（step-2.1） |
| `Dev-Dataflow-ServiceCoding` | [`.github/agents/Dev-Dataflow-ServiceCoding.agent.md`](../.github/agents/Dev-Dataflow-ServiceCoding.agent.md) | バッチ TDD GREEN フェーズ（実装）（step-2.2） |
| `Dev-Dataflow-FunctionsDeploy` | [`.github/agents/Dev-Dataflow-FunctionsDeploy.agent.md`](../.github/agents/Dev-Dataflow-FunctionsDeploy.agent.md) | GitHub Actions CI/CD・Azure Functions デプロイ（step-3） |
...
| Workflow | トリガー | 用途 |
|---|---|---|
| [`auto-dataflow-dev-reusable.yml`](../.github/workflows/auto-dataflow-dev-reusable.yml) | `workflow_call`（dispatcher 経由） | バッチ実装（ADFDV）オーケストレーション本体 |
...
**原因の確認**:
1. `auto-dataflow-dev` ラベルが Issue に付与されているか確認
...
2. `adfdv:done` ラベルが `step-1.2` に付与されているか確認
3. Actions タブで `auto-dataflow-dev-reusable.yml` の状態遷移ジョブが起動しているか確認

---

### ❌ Custom Agent が「依存ファイルが見つからない」と報告する

**原因**: 前段の PR がマージされておらず、入力ファイルが `docs/dataflow/` に存在しない。

**対処**:
1. 前段の PR がマージ済みか確認する
2. `docs/dataflow/` 配下に対象ファイルが存在するか確認する
...
**原因の確認**:
1. Actions タブで `auto-dataflow-dev-reusable.yml` の状態遷移ジョブがエラーで終了していないか確認
2. 前段 Sub Issue が正しく close されているか確認
3. 依存関係がある場合、**すべての依存 Step** が close されているか確認

**対処**:
- Actions のジョブログを確認し、エラーメッセージに従い対処する
- 状態遷移が失敗した場合: Issue に `adfdv:done` ラベルを手動で付与して再トリガーする
...
| ファイル | 用途 |
|---|---|
| [`.github/ISSUE_TEMPLATE/batch-dev.yml`](../.github/ISSUE_TEMPLATE/batch-dev.yml) | ADFDV ワークフロー起動用テンプレート |

### Workflows

| ファイル | 用途 |
|---|---|
| [`.github/workflows/auto-orchestrator-dispatcher.yml`](../.github/workflows/auto-orchestrator-dispatcher.yml) | Issueディスパッチャー（ADFDV 含む各オーケストレーターへ振り分け） |
...
| [`.github/workflows/auto-dataflow-dev-reusable.yml`](../.github/workflows/auto-dataflow-dev-reusable.yml) | バッチ実装（ADFDV）reusable オーケストレーター本体 |

### Custom Agents（ADFDV 実装系）

| ファイル | Step | 成果物 |
|---|---|---|
| [`.github/agents/Dev-Dataflow-DataServiceSelect.agent.md`](../.github/agents/Dev-Dataflow-DataServiceSelect.agent.md) | step-1.1 | `infra/azure/dataflow/create-batch-resources.sh`, `infra/azure/dataflow/verify-batch-resources.sh` |
| [`.github/agents/Dev-Dataflow-DataDeploy.agent.md`](../.github/agents/Dev-Dataflow-DataDeploy.agent.md) | step-1.2 | Azure データリソース実行ログ・検証結果（`work/` 配下） |
| [`.github/agents/Dev-Dataflow-TestCoding.agent.md`](../.github/agents/Dev-Dataflow-TestCoding.agent.md) | step-2.1 | `test/dataflow/{jobId}-{jobNameSlug}.Tests/` |
| [`.github/agents/Dev-Dataflow-ServiceCoding.agent.md`](../.github/agents/Dev-Dataflow-ServiceCoding.agent.md) | step-2.2 | `src/dataflow/` 実装コード |
| [`.github/agents/Dev-Dataflow-FunctionsDeploy.agent.md`](../.github/agents/Dev-Dataflow-FunctionsDeploy.agent.md) | step-3 | `.github/workflows/deploy-batch-functions.yml`, `infra/azure/dataflow/README.md` |
...
| ファイル | 説明 |
|---|---|
| [`users-guide/04-app-design-dataflow.md`](./04-app-design-dataflow.md) | ADFD ワークフロー（ADFDV の前提成果物） |
...
- AI Agent の設計・実装を行う場合は [07-ai-agent-simple.md](./07-ai-agent-simple.md)（AAG 簡易版）または [08-ai-agent.md](./08-ai-agent.md)（AAG / AAGD 本格設計）へ進んでください。
- Workflow 一覧・最新のワークフロー定義は [workflow-reference.md](./workflow-reference.md) を参照してください。
