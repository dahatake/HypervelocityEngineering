...
- [実装・デプロイへの移行（ADFDV 完了後）](#実装デプロイへの移行adfdv-完了後)
...
---
> 初見ユーザーが、**Issue 作成 → Sub Issue 自動生成 → Prompt 実行 → PR マージ** の流れを起点に、ADFDV ワークフローステップ（`step-1.1`〜`step-4.*`）の完了確認から実装・デプロイ移行まで迷わず進められるよう整理したガイドです。
...
![ADFDV アーキテクチャ: 入力ファイル → auto-dataflow-dev Workflow → Prompt チェーン → 成果物](./images/infographic-adfdv.svg)

### データフロー図（ADFDV）

以下の図は、各ステップで Prompt が読み書きするファイルのデータフローを示します。

![ADFDV データフロー: 各 Prompt の入出力ファイル](./images/orchestration-task-data-flow-adfdv.svg)
...
> 📄 テンプレートファイル: [`.github/ISSUE_TEMPLATE/dataflow-dev.yml`](../.github/ISSUE_TEMPLATE/dataflow-dev.yml)
...
## Step 3: Prompt の実行を待つ

各 Sub Issue にアサインされた Copilot は、対応する **Prompt** を使用して設計ドキュメントを生成します。

### 各 Step の Prompt と成果物
...
Prompt が成果物を生成すると、リポジトリに **PR（Pull Request）** が自動作成されます。
...
□ 成果物ファイルが正しいパスに生成されているか
  例: src/infra/azure/, src/test/dataflow/{jobId}-{jobNameSlug}.Tests/, src/dataflow/, docs/azure/
...
<a id="adfdv-ワークフローステップstep-11step-4の完了を確認する"></a>
...
3. 以下の成果物が PR に反映されていることを確認する
   - step-1.1: `src/infra/azure/dataflow/create-batch-resources.sh`, `src/infra/azure/dataflow/verify-batch-resources.sh`（リソース作成・検証スクリプト）
   - step-1.2: Azure データリソースのデプロイ・検証結果（`work/` 配下ログ）
   - step-2.1: `src/test/dataflow/{jobId}-{jobNameSlug}.Tests/`
...
ADFDV 完了後は、必要に応じて運用改善・追加ジョブ実装・監視強化のタスクへ進みます。継続タスクは `create-subissues` フローで分割し、実装は `Dev-Dataflow-*`、レビューは `QA-*` Prompt を使って進めてください（下記「利用可能な Prompt（ADFDV）」参照）。

### Sub Issue を使った実装フェーズの運営

実装フェーズも設計フェーズと同様に、**Sub Issue → Prompt → PR → マージ** のサイクルで進めます。
...
| Agent 名 | ファイル | 主な用途 |
|---|---|---|
| `Dev-Dataflow-DataServiceSelect` | [`.github/prompts/Dev-Dataflow-DataServiceSelect.prompt.md`](../.github/prompts/Dev-Dataflow-DataServiceSelect.prompt.md) | Azure データサービス選定・リソース作成スクリプト生成（step-1.1） |
| `Dev-Dataflow-DataDeploy` | [`.github/prompts/Dev-Dataflow-DataDeploy.prompt.md`](../.github/prompts/Dev-Dataflow-DataDeploy.prompt.md) | Azure データリソースの実行デプロイ・検証（step-1.2） |
| `Dev-Dataflow-TestCoding` | [`.github/prompts/Dev-Dataflow-TestCoding.prompt.md`](../.github/prompts/Dev-Dataflow-TestCoding.prompt.md) | バッチ TDD RED フェーズ（テストコード生成）（step-2.1） |
| `Dev-Dataflow-ServiceCoding` | [`.github/prompts/Dev-Dataflow-ServiceCoding.prompt.md`](../.github/prompts/Dev-Dataflow-ServiceCoding.prompt.md) | バッチ TDD GREEN フェーズ（実装）（step-2.2） |
| `Dev-Dataflow-FunctionsDeploy` | [`.github/prompts/Dev-Dataflow-FunctionsDeploy.prompt.md`](../.github/prompts/Dev-Dataflow-FunctionsDeploy.prompt.md) | GitHub Actions CI/CD・Azure Functions デプロイ（step-3） |
| `QA-AzureArchitectureReview` | [`.github/prompts/QA-AzureArchitectureReview.prompt.md`](../.github/prompts/QA-AzureArchitectureReview.prompt.md) | WAF/ASB 観点のアーキテクチャレビュー（step-4.1） |
| `QA-AzureDependencyReview` | [`.github/prompts/QA-AzureDependencyReview.prompt.md`](../.github/prompts/QA-AzureDependencyReview.prompt.md) | Azure 依存関係・設定整合レビュー（step-4.2） |
...
**対処**:
- 手動でアサインする: Issue 右サイドバー → Assignees → `copilot` を選択し、Copilot セクションで適切な Prompt を選択
...
| ファイル | 用途 |
|---|---|
| [`.github/ISSUE_TEMPLATE/dataflow-dev.yml`](../.github/ISSUE_TEMPLATE/dataflow-dev.yml) | ADFDV ワークフロー起動用テンプレート |
...
| ファイル | Step | 成果物 |
|---|---|---|
| [`.github/prompts/Dev-Dataflow-DataServiceSelect.prompt.md`](../.github/prompts/Dev-Dataflow-DataServiceSelect.prompt.md) | step-1.1 | `src/infra/azure/dataflow/create-batch-resources.sh`, `src/infra/azure/dataflow/verify-batch-resources.sh` |
| [`.github/prompts/Dev-Dataflow-DataDeploy.prompt.md`](../.github/prompts/Dev-Dataflow-DataDeploy.prompt.md) | step-1.2 | Azure データリソース実行ログ・検証結果（`work/` 配下） |
| [`.github/prompts/Dev-Dataflow-TestCoding.prompt.md`](../.github/prompts/Dev-Dataflow-TestCoding.prompt.md) | step-2.1 | `src/test/dataflow/{jobId}-{jobNameSlug}.Tests/` |
| [`.github/prompts/Dev-Dataflow-ServiceCoding.prompt.md`](../.github/prompts/Dev-Dataflow-ServiceCoding.prompt.md) | step-2.2 | `src/dataflow/` 実装コード |
| [`.github/prompts/Dev-Dataflow-FunctionsDeploy.prompt.md`](../.github/prompts/Dev-Dataflow-FunctionsDeploy.prompt.md) | step-3 | `.github/workflows/deploy-batch-functions.yml`, `src/infra/azure/dataflow/README.md` |

### Prompt（QA）

| ファイル | 用途 |
|---|---|
| [`.github/prompts/QA-AzureArchitectureReview.prompt.md`](../.github/prompts/QA-AzureArchitectureReview.prompt.md) | WAF/ASB 観点のレビュー |
| [`.github/prompts/QA-AzureDependencyReview.prompt.md`](../.github/prompts/QA-AzureDependencyReview.prompt.md) | 依存関係・設定整合レビュー |
