...
> **補足**: `qa-requirement-classification.yml` および `self-improve.yml` については、`.github/workflows/` 配下に対応する GitHub Actions ワークフローが存在しないため、Issue 作成のみでは Web UI から自動実行されません。これらを実行する場合は、Issue に対して手動で `@copilot` をアサインするか、GitHub Copilot CLI SDK 版から実行してください。
