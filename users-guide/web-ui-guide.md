...
- [利用可能な Prompt 一覧](#利用可能な-prompt-一覧)
- [ヒント](#ヒント)
...
| 確認項目 | 確認方法 |
|---|---|
| [hve-cloud-getting-started.md](./hve-cloud-getting-started.md) の HVE Cloud Agent Orchestrator 初回セットアップが完了している | [チェックリスト](./hve-cloud-getting-started.md#hve-cloud-agent-orchestrator-初回セットアップ-チェックリスト) を参照 |
...
> **問題が発生した場合**: [troubleshooting.md](./troubleshooting.md) を参照してください。初回セットアップに不明点がある場合は [hve-cloud-getting-started.md](./hve-cloud-getting-started.md) に戻って確認してください。
...
- **方式1: Copilot cloud agent 手動実行** — GitHub.com 上で Issue を個別に作成し、Prompt を選択して Copilot をアサインする最小単位の使い方
...
> [!NOTE]
> 全体像と 4 つの使い方の比較は [README.md](../README.md#方式比較表4-つの使い方) を参照してください。
...
> **HVE CLI Orchestrator（ローカル実行）** との比較は [README.md](../README.md#方式比較表4-つの使い方) を参照してください。
...
>
> 初回セットアップがまだの場合は、先に [hve-cloud-getting-started.md の Step.5](./hve-cloud-getting-started.md#step5-ラベル設定) を完了してください。

---

### 方式1: Copilot cloud agent 手動実行

1 つの Issue に 1 つの Prompt を割り当て、Copilot に個別のタスクを実行させる最もシンプルな使い方です。
...
Issue 作成時または `@copilot` へのアサイン時に、右側サイドバーの **「Copilot」** セクションで **「Select agent」** から使用したい Prompt を選択します。

![Prompt の選択](./images/assign-githubcopilot-customagent.jpg)

> **手動アサインの場合**: Issue 右サイドバーの「Assignees」から `@copilot` を選択、または Issue のコメント欄で `@copilot` をメンションしてください。

#### Step.3. タスクの詳細を Issue に記述する

Prompt が適切に動作するために、タスクの詳細・要件・参照すべきファイルパスなどを明確に記述してください。
...
- 選択した Prompt がタスクを実行し、Pull Request を作成します
...
> **応用パターン**: 複数の Prompt を使ったフルワークフローを手動で実行したい場合は、DAG の依存関係順（[workflow-reference.md](./workflow-reference.md) 参照）に従って Issue を順番に作成・アサインしてください。
...
> [!IMPORTANT]
> 方式2 を使用するには `COPILOT_PAT` シークレットが設定されている必要があります。設定がまだの場合は [hve-cloud-getting-started.md の Step.4](./hve-cloud-getting-started.md#step4-認証設定copilot_pat) を完了してください。
...
Bootstrap Workflow が自動的に Sub Issue を生成し、Copilot を各 Sub Issue に自動アサインします。特定の Sub Issue のみ別の Prompt を使いたい場合は、対象の Sub Issue の右サイドバーから **「Select agent」** で変更できます。

![Prompt の選択](./images/assign-githubcopilot-customagent.jpg)

#### Step.3. タスクの詳細を親 Issue に記述する

親 Issue に記述した内容をもとに、Bootstrap Workflow が各 Sub Issue を生成します。Prompt が適切に動作するために、タスクの詳細・要件・参照すべきファイルパスなどを明確に記述してください。
...
- **適切な Prompt を選択**: タスクの内容に応じた Prompt を選択することで、より高品質な結果が得られます
- **段階的に進める**: 大きなプロジェクトは複数の Prompt を順番に使用して段階的に進めることを推奨します
- **カスタマイズ**: Prompt ファイルはテンプレートです。プロジェクトの要件に応じて自由に編集・追加できます
- **フィードバック**: Prompt の実行結果を確認し、必要に応じて Issue のコメントで追加指示を出せます
