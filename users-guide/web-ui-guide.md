...
> **SDK 版（ローカル実行）** との比較は [Users Guide](./README.md#2-つの利用方法) を参照してください。
...
| Issue テンプレート | ワークフロー |
|-----------------|------------|
| `auto-app-selection.yml` | アプリケーション選定（AAS） |
| `auto-app-design.yml` | アプリケーション設計（AAD） |
| `auto-app-dev-microservice.yml` | マイクロサービス実装（ASDW） |
| `auto-batch-design.yml` | バッチ設計（ABD） |
| `auto-batch-dev.yml` | バッチ実装（ABDV） |
| `auto-iot-design.yml` | IoT 設計（AID） |
