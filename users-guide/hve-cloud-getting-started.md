...
| 引数 | 動作 |
|---|---|
| なし（既定） | venv 作成 + SDK + mdq extras + GUI extras を導入 |
| `--check-only` | 状態確認のみ（変更なし） |
| `--skip-mdq` | mdq extras をスキップ |
| `--no-gui` | GUI extras をスキップ（CLI のみ） |
| `--help` / `/?` | 使い方表示 |

> **`.ps1` との違い**: `.cmd` は GUI extras を **既定で導入** します（`.ps1` は `-WithGui` 指定時のみ）。`-WithWorkIQ` / `-InstallExternalCopilotCli` / `-ForceRecreateVenv` / `-SkipMdqWatch` 等の上級オプションは `.cmd` ではサポートしません。これらが必要な場合は `hve\setup-hve.ps1` を使用してください（[詳細](./hve-cli-orchestrator-guide.md#セットアップスクリプトを使った環境構築windows--macos--linux)）。
...
`-WithGui` / `--with-gui` と組み合わせ可能です:

```powershell
powershell -ExecutionPolicy Bypass -File hve\setup-hve.ps1 -WithGui -WithSkills
```
...
HVE GUI Orchestrator の ARD ワークフローで `.docx` / `.pdf` / `.xlsx` / `.xls` / `.pptx` / `.html` をドラッグ&ドロップで Markdown 化するには、`gui` + `gui-docconvert` extras（変換エンジンは [microsoft/markitdown](https://github.com/microsoft/markitdown)）が必要です。セットアップスクリプトに `--with-gui` / `-WithGui` を渡すと自動でインストールされます。
