# hve アプリケーション ユーザーガイド（hve アプリケーション版ワークフローオーケストレーション）
...
- [はじめに](#はじめに)
- [クイックスタート](#クイックスタート)
- [セットアップスクリプトを使った環境構築](#セットアップスクリプトを使った環境構築windows--macos)
...
本ガイドは **hve アプリケーション（ローカル実行方式）** に特化しています。Web UI 方式との比較や全体の利用ガイドについては [README.md](../README.md#3-つの使い方) を参照してください。
...
## セットアップスクリプトを使った環境構築（Windows / macOS）
...
`AQOD` では wizard 経由で実行した場合、`aqod` + `auto_qa=True` の組み合わせで `workiq_draft_mode` が自動的に ON になります（質問なし）。この場合でも Work IQ は AQOD メインタスクの `original-docs/` 整合性レビューでは使用しません。
...
```text
? ワークフローを選択してください
  1) App Architecture Design (aas — 8 steps)
  2) Web App Design (aad-web — 4 steps)
  3) Web App Dev & Deploy (asdw-web — 19 steps)
  4) Batch Design (abd — 9 steps)
  5) Batch Dev (abdv — 7 steps)
  6) AI Agent Design (aag — 3 steps)
  7) AI Agent Dev & Deploy (aagd — 5 steps)
  8) Knowledge Management (akm — 1 step)
  9) Original Docs Review (aqod — 1 step)
  10) Source Codeからのドキュメント作成 (adoc — 23 steps)
> 2
```
...
> 固有パラメータのないワークフロー（`aas`, `aad`）ではこのステップはスキップされます。`abd` は `app_ids`, `app_id` パラメータを持つようになりました。
...
#### aad — App Detail Design（アプリケーション設計）
...
hve アプリケーション版でワークフローを実行する際も、`knowledge/` フォルダーの業務要件ドキュメント（D01〜D21）が存在する場合、各 Custom Agent が自動参照します。hve アプリケーション版での `knowledge-management` ワークフロー実行:
