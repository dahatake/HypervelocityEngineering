...
wizard は以下の段階で進行します。ステップ 4（モデル選択）の直後に **実行モード選択** が追加されています。
...
> **注意**: クイック全自動でも、AQKM 以外のワークフローで**必須パラメータ**（`app_id`、`usecase_id` 等）がある場合は、それらの入力のみ求められます。
...
[14:30:20]   📊 [1.1] claude-opus-4.6 in=8500 out=2450 3200ms
