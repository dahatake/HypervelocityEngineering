...
Work IQ（`@microsoft/workiq`）をインストールして `--workiq` を有効化すると、QA / AKM / AQOD の補助情報として M365 データを読み取り専用で参照します。  
QA では `--workiq-draft` を指定すると、質問ごとの Work IQ 回答ドラフトを `qa/`（または指定ディレクトリ）へ出力できます。
...
#### Work IQ が提供するツール一覧（読み取り専用）

`build_workiq_mcp_config()` の実装に基づく:
...
```text
hve wizard / CLI
  -> Work IQ 利用有無判定（未インストール時はスキップ）
  -> npx @microsoft/workiq mcp を _hve_workiq として注入
  -> QA: 質問票を要約して Work IQ 問い合わせ（通常モード）
  -> QA(ドラフトモード): 質問ごとに Work IQ を実行し、回答ドラフトを qa/*.md に保存
  -> AKM/AQOD: step prompt に Work IQ 問い合わせ指示を事前注入
  -> QA のみ(通常モード): 取得結果を delimiters 付きで本プロンプトへ注入（外部命令は無視）
```
...
プロンプトカスタマイズ（CLI / 環境変数）:

| 用途 | CLI 引数 | 環境変数 |
|---|---|---|
| 有効化 | `--workiq` | `WORKIQ_ENABLED=true` |
| QA 回答ドラフト有効化 | `--workiq-draft` | `WORKIQ_DRAFT_MODE=true` |
| QA 回答ドラフト出力先 | `--workiq-draft-output-dir` | `WORKIQ_DRAFT_OUTPUT_DIR` |
| テナントID | `--workiq-tenant-id` | `WORKIQ_TENANT_ID` |
| QA プロンプト | `--workiq-prompt-qa` | `WORKIQ_PROMPT_QA` |
| AKM プロンプト | `--workiq-prompt-km` | `WORKIQ_PROMPT_KM` |
| AQOD プロンプト | `--workiq-prompt-review` | `WORKIQ_PROMPT_REVIEW` |
...
? ワークフローを選択してください
  ...
  2) App Detail Design (aad — 16 steps)
  ...
  7) Original Docs Review (aqod — 1 step)
  8) Source Codeからのドキュメント作成 (adoc — 23 steps)
> 2
```
