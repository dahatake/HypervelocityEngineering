...
- Workflow ID / Prompt: `aqod` / `QA-DocConsistency`（`hve/workflow_registry.py`）
...
Prompt の入出力ファイルを示します（`hve/workflow_registry.py` の AQOD 定義に準拠）。

![AQOD: Original Docs Review データフロー](./images/orchestration-task-data-flow-aqod.svg)
...
共通手順は [hve-cloud-getting-started.md](./hve-cloud-getting-started.md) / [hve-cli-getting-started.md](./hve-cli-getting-started.md) / [hve-gui-getting-started.md](./hve-gui-getting-started.md) を参照してください。
