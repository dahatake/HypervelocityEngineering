...
- **セグメントは連続一致を要求する**。空白を含まない日本語クエリは全体がそのまま現れる文書しかヒットしない。in-memory BM25（語を 1 文字へ分解して OR 相当）より **precision は高く recall は低い**。幅広く拾いたい場合は語を空白で区切るか `--engine bm25` を使う。
...
- **in-memory BM25**（既定）: クエリ時に全チャンクをメモリへロード。小〜中規模コーパスで十分実用。stdlib のみで動作する。ただし CJK を 1 文字へ分解するため、語の連続性を見ない（文字の出現回数だけで順位が決まる）。
