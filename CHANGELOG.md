...
- **`hve/mcp_io_log.py` を新規追加した**。`McpIoLogger` は MCP サーバーごとに `work/run/<run-id>/mcp-<サーバー名>.log` を遅延 open して追記する。記録するのは `mcp_request`（MCP ツールの引数）/ `mcp_response`（結果本文またはエラー）/ `mcp_server_status`（接続状態・プラグイン名・トランスポート）/ `session_prompt`（Work IQ へ送ったプロンプト全文）/ `session_response`（その応答全文）の 5 種別で、各レコードは `=== <時刻> | <種別> | server=<名前> | ...` の 1 行ヘッダと本文からなる。表示用の切り詰めは適用しない。あわせて、`StepRunner` を経由しないセッションへ結線するための `attach_mcp_io_event_logger()` を同モジュールへ用意した。
- **完了イベントの帰属を `tool_call_id` 相関のみに限定した**。SDK 1.0.9 の `ToolExecutionCompleteData` は `mcp_server_name` を持たないため（`tool.execution_start` 側の `ToolExecutionStartData` のみが持つ）、相関できない完了イベントは MCP 由来か組み込みツール由来かを判別できない。相関に失敗した完了は記録しない。
- **`hve/runner.py` の SDK イベント分岐 4 箇所から記録するようにした**。`tool.execution_start`（`mcp_server_name` を持つもののみ）・`tool.execution_complete`・`session.mcp_servers_loaded`・`session.mcp_server_status_changed`。MCP の記録は `report_intent` / `task` の早期 return より前に置いた。後ろに置くと、組み込みツールと同名の MCP ツールが公開された場合にレコードが無言で欠落する。
- **`hve/console.py` に記録器の接続と 3 つの記録メソッドを追加し、`workiq_prompt` / `workiq_response` が verbosity 判定より前に全文を記録するようにした**。`quiet`（verbosity 0）や `final_only` でも記録は継続し、人間向け表示だけが抑止される（FR-RTO-02 と同じ「収集・保存」と「表示」の分離方針）。
- **`hve/orchestrator.py` に `_attach_mcp_io_logging()` を追加し、`run_workflow` の atexit と正常終了の双方で close するようにした**。あわせて、`StepRunner` を経由しない Work IQ 専用セッション 4 箇所（prefetch / AKM verification / AKM ingest / ARD usecase）へ記録経路を結線した。ARD usecase 経路だけ `console.workiq_prompt` の呼び出しを欠いており、この経路のプロンプトだけが表示にも記録にも残らなかったため、他 3 経路と対称になるようプロンプトと応答の出力を追加した。
- **親起動子プロセスの判定を `hve/runtime_observability.py` の `is_child_process()` へ単一実装として集約した**（FR-MAINT-07）。作業フォルダーを共有する子プロセスでは `mcp-<サーバー名>-<pid>.log` へ分離する。1 レコードが複数行にわたるため、複数プロセスが同一ファイルへ追記するとレコードが壊れるためである。GUI Autopilot の子は `HVE_GUI_SESSION_ID` だけを、CLI Autopilot の子は `HVE_STATS_STREAM` だけを継承するため、いずれか一方だけを条件にすると他方で交錯する。`hve/console.py` の `[hve:stats]` 子プロセス配信可否も同じ実装を参照するよう置き換えた（式は従来と同値）。
- **秘密情報のマスクは既存実装へ委譲した**（FR-MAINT-07）。`hve/workiq.py` の `_sanitize_diagnostic_text`（`Authorization: Bearer` / `token=` 等の key-value / JWT を `[REDACTED]` へ置換）を再利用し、同等処理を新規実装していない。
- **新規の CLI オプション・設定項目・環境変数を追加していない**。有効化条件は既存の実行時 Observability と同じく `HVE_WORK_ROOT` が設定された非 dry-run 実行のみとした。上限は 1 ファイル 32 MiB で、到達時は追記を停止して警告を 1 回だけ出す（ローテーションなし）。書き込み失敗は Step へ波及させない。
- **`hve-dev/requirement-definition.md` へ §3.13（FR-MCPLOG-01〜03）を新設し、改訂履歴へ v2.49 を追加した**。本ログは prompt 本文・tool 引数・応答本文を意図的に保持するため、observability ストア（`observability/events-<pid>.jsonl`）の allowlist を定める FR-RTO-04 は適用しないことを明示的に切り分けた。FR-RTO-01〜07 は改訂していない。
- **`hve-dev/requirement-test-mapping.md` へ 3 要件分のテスト仕様と GREEN 結果を追加し、`hve-dev/` の索引 3 種を再生成した**。
- **`users-guide/` を 3 ファイル更新した**。`hve-cli-orchestrator-guide.md` に「MCP 通信ログ」節（出力先・レコード種別・Microsoft 365 Copilot Chat での再利用手順・取り扱い上の注意）を追加し、`troubleshooting.md` の初期セットアップ確認観点と「Work IQ の回答案が QA へ 1 件も統合されない」節へ切り分け導線を追加し、`01-business-requirement.md` の `work/run` 成果物一覧へ本ログを追記した。

**影響範囲**: 新規 `hve/mcp_io_log.py`、`hve/console.py`、`hve/runner.py`、`hve/orchestrator.py`、`hve/runtime_observability.py`、新規テスト 4 ファイル（`hve/tests/test_mcp_io_log.py`・`test_console_mcp_io_log.py`・`test_runner_mcp_io_log.py`・`test_orchestrator_mcp_io_log.py`）、`hve-dev/requirement-definition.md`・`requirement-test-mapping.md`・索引 3 種、`users-guide/hve-cli-orchestrator-guide.md`・`troubleshooting.md`・`01-business-requirement.md`。`hve/__main__.py`・`hve/config.py`・`mdq/`・`cq/`・`hve/toolsearch/`・`.gitignore` は本変更では触れていない。

**検証**: 新規テスト 69 件（`test_mcp_io_log.py` 37 / `test_console_mcp_io_log.py` 12 / `test_runner_mcp_io_log.py` 9 / `test_orchestrator_mcp_io_log.py` 11）がすべて成功することを確認した。実装前に RED（`ImportError: cannot import name 'mcp_io_log'`）を確認してから実装している。回帰は `python -m pytest hve/tests -q` で **8463 passed, 17 skipped, 1 xfailed, 697 subtests passed（20 分 14 秒）** を確認した。同時に 1 件 failed（`test_prompts.py::TestOverEngineeringBan::test_fanout_common_files_contain_oe_ban` の `11 != 12`）が出たが、これは同一ワークツリーで並行して行われている `hve/prompt/fanout/aas/_common.md` の未コミット削除に起因するもので、本変更が触れていないファイルの問題である（`git status --porcelain -- hve/prompt` が ` D hve/prompt/fanout/aas/_common.md` を示す）。新規追加ファイルに lint エラーが無いことも確認した。

**既知の制約**: MCP サーバープロセスは Copilot CLI ランタイムが起動し HVE はその標準入出力を保持しないため、生の JSON-RPC フレームは取得できない。記録できるのは SDK イベントが公開する上記 5 レコード種別に限られる。`ToolExecutionCompleteResult` からは `content` のみを記録し、`detailed_content` / `structured_content` は対象外とした。本ログは M365 の業務データを平文で含む（`.gitignore` の `*.log` によりコミットはされない）。既存マスク実装は完全なサニタイズを保証しない。外部 Copilot CLI へ委譲する GUI の対話セッション（`hve/gui/copilot_interactive_session.py`）は HVE 側に SDK イベント経路が無いため対象外とした。

<!-- validation-confirmed -->

### Fixed — GUI が起動する HVE サブプロセスの標準入力を対話不能にし、Work IQ 認証 preflight で GUI が応答不能のまま停止する事象を解消した（FR-GUI-23）

GUI から ARD ワークフローを起動したところ、「ヘッドレス環境を検出しました」の警告の後に GUI が無反応になり、停止するまで進まない事象が報告された。調査の結果、独立した 4 件の欠陥が連鎖していた。(1) `launch_orchestrator()` と Autopilot の `_default_popen()` が `stdin` を指定せず、ターミナルから起動した GUI の端末標準入力を子プロセスへ継承させていたため、CLI 側 preflight の `input()` でブロックした。GUI 側に子プロセスへ入力を送る経路は存在せず、応答不能だった。(2) `input()` のプロンプトは改行を伴わないため、GUI の行単位読み取りでは表示されず、利用者には無音のフリーズに見えていた。(3) `_is_headless_environment()` が macOS を常にヘッドレスと誤判定し、ブラウザ認証を一切試みずに Work IQ 認証を失敗扱いにしていた。(4) Work IQ を要求した設定が画面から判別できず、利用者は「Work IQ をオフにしたのに preflight が走った」状態の原因を特定できなかった。

- **GUI が起動する HVE サブプロセスの標準入力を `subprocess.DEVNULL` にした**（[hve/gui/state_bridge.py](hve/gui/state_bridge.py) `launch_orchestrator` と [hve/gui/autopilot/child_launcher.py](hve/gui/autopilot/child_launcher.py) `AutopilotController._default_popen` の 2 箇所）。これにより `sys.stdin.isatty()` が偽となり、[hve/__main__.py](hve/__main__.py) の対話プロンプト 4 箇所（`_run_copilot_auth_preflight` / `_run_workiq_auth_preflight` / `_run_azure_auth_preflight` / `_confirm_autopilot_chain_start`）が一括で非対話経路へ落ちる。失敗時は理由を実行ログへ出力して終了コード 1 で終了するため、GUI が停止しない。CLI 単体実行の対話可否判定規則は変更していない。
- **preflight ごとに GUI 判定分岐を追加する方式は採用しなかった**。同じ判定を 3 箇所へ分散させることになり、新しいプロンプトを追加するたびに再実装が必要になるため（FR-MAINT-07）。
- **`_is_headless_environment()` の macOS 誤判定を修正した**（[hve/workiq.py](hve/workiq.py)）。macOS は Quartz ベースで `DISPLAY` を使わないため、`DISPLAY` / `WAYLAND_DISPLAY` 未設定をヘッドレスの根拠にできない。判定式へ `sys.platform != "darwin"` を 1 条件追加するに留め、SSH 経由の macOS（`SSH_TTY` / `SSH_CLIENT`）と Linux / WSL（`DISPLAY` 未設定）の既存挙動は維持した。
- **Work IQ 認証 preflight が非対話で失敗したとき、要求元の設定名を 1 行出力するようにした**（[hve/__main__.py](hve/__main__.py)）。判定と理由列挙を 2 重実装しないよう、`_workiq_request_reasons()` を単一の情報源として `_is_workiq_requested()` をその上へ再定義した。出力は失敗時の 1 行のみで、正常系のログは増やしていない。
- **`workiq_akm_ingest_enabled` を AKM 以外の Workflow で要求扱いにしないようにした**（[hve/__main__.py](hve/__main__.py)）。本フラグの機能的な消費点は [hve/orchestrator.py](hve/orchestrator.py) の 2 箇所だけで、いずれも `workflow_id == "akm"` で gate 済みであることを確認した。GUI の取り込みソース設定（AKM 専用 UI）は全 Workflow で `--sources` として argv に乗るため、ARD 実行でも画面に現れないまま Work IQ を要求扱いにできた。対象 Workflow を CLI 引数が不在の対話ウィザード経路でも正しく伝搬するため、`_run_workiq_auth_preflight()` へ `workflow` キーワードを追加し、ウィザード側から `wf.id` を渡すようにした。
- **`users-guide/hve-gui-orchestrator-guide.md` の「トークン失効への対策」節を実装状況へ訂正した**。記載されていた `AuthMonitor`（「5 分ごとの heartbeat」「実行直前の再確認」「実行中の失効検知」）はリポジトリ全体で当該ドキュメント 1 行にしか出現せず、実装が存在しないことを確認した。実際の挙動（手動確認のみ、CI/CD 有効時の `GH_TOKEN` 検査だけが実行前チェック、それ以外は実行開始後の preflight が確認）へ置き換えた。
- **要求定義とテストマッピングを更新した**。`hve-dev/requirement-definition.md` に §6.12（FR-GUI-23）を新設し、改訂履歴 2.48 を追記した。`hve-dev/requirement-test-mapping.md` に FR-GUI-23 の受入テストと RED / GREEN 証跡を記録した。

**影響範囲**: `hve/gui/state_bridge.py`、`hve/gui/autopilot/child_launcher.py`、`hve/workiq.py`、`hve/__main__.py`、`hve/gui/tests/test_gui_subprocess_stdin.py`（新規）、`hve/gui/tests/test_autopilot_child_launcher.py`、`hve/tests/test_workiq.py`、`hve/tests/test_main.py`、`hve-dev/requirement-definition.md`、`hve-dev/requirement-test-mapping.md`、`users-guide/hve-gui-orchestrator-guide.md`、`users-guide/hve-cli-orchestrator-guide.md`、`users-guide/hve-technical-architecture.md`。`hve/orchestrator.py` / `hve/runner.py` / `hve/config.py` / GUI の設定既定値（`hve/gui/settings_store.py`）は変更していない。翻訳カタログ（`.ts` / `.qm`）は Qt の `tr()` 文言を変更していないため未変更。

**既知の制約**: (1) `hve/gui/state_bridge.py` の `SubprocessReader.stop()` が GUI メインスレッドで最大 8 秒ブロックする点は本変更の対象外とした。今回の停止事象の主因ではなく、修正には `stop_orchestrator()` の同期戻り前提と Autopilot 停止経路へ波及するため。(2) `_has_cached_token()` の macOS キャッシュパス追加は対象外とした。`@microsoft/workiq` が macOS でトークンをどこへ置くかを未確認であり、確認せずに候補パスを増やすことはできないため（macOS のヘッドレス誤判定を直したことで、通常のブラウザ認証経路へ入る）。(3) `hve/gui/toolsearch_settings_section.py` の `python -m hve toolsearch context` は `orchestrate` ではなく preflight を通らないため対象外とした。(4) `hve-dev/` の索引再生成は 3 回目で収束した（`hve/tests/test_hve_surface_inventory.py` **147 passed**）。1 回目・2 回目は同一ワークツリーで並行セッションが `hve/runner.py` 等を編集中だったため、再生成直後に `test_csv_is_not_stale` が自分の触っていないシンボルの行番号ずれ（`hve/runner.py` の 7559 → 7561）で再失敗した。索引は生成物のため、以降の並行編集で再び stale になった場合は後続セッションの再生成で解消する。

**検証**: TDD 順序で RED → GREEN を実測した。RED: `test_gui_subprocess_stdin.py` / `test_autopilot_child_launcher.py::test_default_popen_disables_stdin` が `KeyError: 'stdin'` で **2 failed**、`test_workiq.py` の macOS / Linux 判定 3 件が `AttributeError: module 'workiq' has no attribute 'sys'` で **3 failed**、`test_main.py::TestWorkIQAuthPreflight::test_non_interactive_failure_reports_request_source` が AssertionError で **1 failed**、`test_akm_ingest_only_skips_preflight_for_non_akm` が `Expected 'workiq_login' to not have been called` で **1 failed**。GREEN: `test_gui_subprocess_stdin.py` / `test_autopilot_child_launcher.py` / `test_start_autopilot_chain_branch.py` / `test_workbench_window_observability.py` で **22 passed**、`test_workiq.py` / `test_main.py` / `test_gui_imports.py` / `test_akm_workiq_ingest.py` で **512 passed, 74 subtests passed**、`test_main_ard.py` / `test_qa_gui_file_mode.py` で **56 passed**。RED 証跡は実装を一時的に差し戻して実測した（git 操作は行っていない）。
...
- **完了後の横断チェックで発見した 3 件の残存不整合を修正した**。(1) `README.md` のディレクトリ表で `src/data/` を「AAS Step 4.2 出力」としていた旧 ID 参照を Step 3.2 へ訂正した。(2) `hve-dev/requirement-test-mapping.md` の FR-AZFREE 既知の制約欄にあった「`aas`（11 中 1 Step）」を「10 中 1 Step」へ訂正した。(3) `.github/scripts/README.md` の「サポートするワークフロー」表を registry の実データへ全面的に差し替えた（従来は 10 行中 6 行のステップ数が誤り、`ard` / `ada` / `aar` が未掲載、逆に本ディレクトリのスクリプトに未登録の `aad-web` / `akm` / `adi` が掲載されていた）。
- **`.github/scripts/README.md` の実行不能なサンプルコマンドを修正した**。`orchestrate.sh --workflow aad` / `orchestrate.ps1 -Workflow aad` は `ERROR: 不明なワークフロー: aad` で失敗することを実行確認し、`--steps` の使用例として同等の意味を持ち両シェルで成功する `adfdv --steps 1.1,1.2` へ置き換えた。あわせて、`orchestrate` が registry より狭い専用 allowlist を持つこと（Bash 4 件 / PowerShell 3 件）と、`advance` は allowlist を持たず registry を直接引くため扱える ID が registry の登録内容と等しいこと（`advance.sh` 10 件 / `advance.ps1` 4 件）を、いずれも実測のうえで注記した。
- **コードコメントに残っていた旧 AAS Step ID を修正した**。`hve/workflow_registry.py` の ADA 定義直前にある「AAS から除外した 3 Step」解説（`Step.2` / `Step.6` / `Step.9` → `Step.1` / `Step.5` / `Step.8`）と、`hve/tests/test_input_artifact_check.py` の docstring 例示を新 ID へ揃えた。あわせて、同コメント内で AAS と ADA の Step 番号が区別しづらかった 2 箇所へ `ADA` 接頭辞を付けた。いずれもコメントのみで振る舞いは変えていない。
- **旧 fan-out 構成の残骸 `hve/prompt/fanout/aas/_common.md` を削除した**。AAS はどの Step にも `fanout_parser` / `fanout_static_keys` / `additional_prompt_template_path` を持たず（実測 0 件）、当該ファイルはどこからも読み込まれない死んだ成果物でありながら、本文は「AAS Step 2 の fan-out 子」と称して FR-WF-AAS-02（AAS Step 1 は APP 単位に fan-out しない）と矛盾していた。あわせて `hve/tests/test_prompts.py` の `_common.md` 件数ガードを 12 → 11 へ更新した（同テストの本来の検証対象である OE 禁止マーカーの全件チェックは維持）。
- **ルート直下の孤立スナップショット `tracked_github.txt` を削除した**。`git ls-files -- .github` の出力を 691 行そのまま保存したファイルで、先行コミット `0da8f77f` に偶発的に含まれたもの。どの追跡ファイルからも参照されず、`protect-readonly-paths.yml` の `ROOT_FILE_ALLOWLIST` にも未登録で、AAS リナンバリングで消えた io-contract / template のパスを 6 件含む陳腐化状態だった。削除によりルート直下ファイル規約にも適合する。
- **`hve-dev/requirement-definition.md` の FR-AZFREE 根拠文にある実測値を現状へ更新した**（`ard` 8 → 10 Step、対象計 42 → 44 Step、`aas` 11 → 10 Step）。allowlist（`ard` / `akm` / `adi` / `adoc`）という結論自体は変わらない。

**影響範囲**: `users-guide/00-design-doc-ingestion.md`・`01-business-requirement.md`・`02-app-architecture-design.md`・`hve-cli-orchestrator-guide.md`・`hve-gui-orchestrator-guide.md`・`workflow-reference.md`・`users-guide/images/{chain-aas,infographic-aas,orchestration-task-data-flow-aas}.svg`、`hve/tests/{test_consumed_artifacts,test_dag_planner,test_data_model_split_contract,test_prompts}.py`、`hve/tests/test_input_artifact_check.py`（docstring のみ）、`hve/workflow_registry.py`（コメントのみ。StepDef 定義・DAG・成果物パスは無変更）、`hve/gui/tests/test_workflow_step_requirements.py`（コメントのみ）、`hve/prompt/fanout/aas/_common.md`（削除）、`tracked_github.txt`（削除）、`.github/scripts/powershell/tests/{workflow-registry,commands}.Tests.ps1`、`hve-dev/{hve-feature-inventory.csv,hve-test-inventory.csv,hve-tdd-crosswalk-baseline.md,requirement-test-mapping.md,requirement-definition.md}`、`README.md`、`.github/scripts/README.md`。`.github/prompts/`・`.github/io-contracts/`・`orchestrate.sh` / `orchestrate.ps1` 本体は変更していない。

**既知の制約**: ADA ワークフロー（`Step 2` から始まる同種の歯抜け構成）のリナンバリングは対象外（要求定義書 FR-WF-ARD-04 の記載どおり、AAS のみを対象とする措置）。`orchestrate.sh` / `orchestrate.ps1` が registry より狭い対応範囲しか持たない点（`ard` / `ada` / `aar` / `aag` / `aagd` / `asdw-web` を受け付けない）は今回ドキュメントへ明記するに留め、スクリプト側の対応拡張は行っていない。

**検証**: `hve/tests` 全体を実行し、修正前は AAS Step ID リナンバリングに起因する **8 failed**（`test_aas_step4_uses_service_catalog` 等）を確認し、修正後は **8389 passed, 17 skipped, 1 xfailed, 697 subtests passed**（実行時間 13分39秒、新規失敗なし）を確認した。PowerShell: `Invoke-Pester` で `.github/scripts/powershell/tests` 全体 **83 passed, 0 failed**（修正前は AAS 関連 2 件が failed）、`Invoke-ScriptAnalyzer`（`PSUseBOMForUnicodeEncodedFile` 除外）で新規警告 0 件（既存 3 件の `PSUseDeclaredVarsMoreThanAssignment` は本セッション以前から存在し無関係であることを HEAD 版との比較で確認済み）を確認した。3 枚の SVG は `xml.etree.ElementTree` で整形式であることを確認した。`.github/scripts/README.md` の表は、bash registry の JSON を直接パースした総 Step 数・名称、`orchestrate.sh` / `orchestrate.ps1` への全 ID 実投入結果（受理 / `不明なワークフロー`）、`advance.sh` が allowlist を持たず `get_workflow` を直接呼ぶことのソース確認、という 3 系統の実測で全セルを裏付けた。差し替え後のサンプルコマンドは Bash / PowerShell の両方で実行し `EXIT=0` を確認した。コメント / docstring 修正後は対象を直接カバーするサブセット（`test_input_artifact_check` / `test_workflow_registry` / `test_ada_workflow` / `test_consumed_artifacts` / `test_dag_planner` / `test_data_model_split_contract` / `test_dag_parity`）で **409 passed, 1 skipped, 1 xfailed** を確認した。fan-out 残骸削除・`tracked_github.txt` 削除・FR-AZFREE 実測値更新の後は `test_prompts` / `test_fanout` / `test_workflow_registry` / `test_hve_requirement_traceability_contract` / `test_requirement_section13_parity` で **397 passed, 1 skipped** を確認し、旧 AAS Step 参照の横断スイープ（追跡 2,663 ファイル）が実在ヒット 0 件（残り 4 件はいずれも新 ID・履歴対応表の正当な記述）になったことを確認した。なお最終確認として再実行した `hve/tests` 全体は、本変更と無関係な Qt/GUI 由来の非決定的プロセスクラッシュ（stderr 空・`FAILED` 記録 0 件・進捗途中で終了）が連続したため完走サマリーを取得できていない。全体 GREEN の根拠は上記の 8389 passed（同一セッション内の完走実行）とする。
