"""AAS Step.8 / Step.9 自然順採番の全実行面契約テスト（FR-WF-AAS-01）。

契約:
    Step 8 = ペルソナカタログ（`Arch-PersonaCatalog` / depends_on=["7"]）
    Step 9 = ペルソナ別共通画面カタログ（`Arch-UI-PersonaScreenList` / depends_on=["8"]）
...
本ファイルは再採番実装の前に先行作成された契約テストであり、実装前は
旧採番（Step 9=ペルソナカタログ / Step 8=ペルソナ別共通画面）のみを理由として
RED になる。各面の再採番完了後に GREEN へ変わることを受入条件とする。
"""
...
PERSONA_CATALOG_STEP = "8"
PERSONA_SCREEN_STEP = "9"
...
class TestRegistryContract:
    """Python registry が Step 8/9 の意味と依存を自然順で宣言すること。"""

    def test_step_8_is_persona_catalog(self) -> None:
        step = get_step("aas", PERSONA_CATALOG_STEP)
        assert step is not None
        assert step.title == "ペルソナカタログ"
        assert step.custom_agent == PERSONA_CATALOG_AGENT
        assert step.depends_on == ["7"]
        assert step.skip_fallback_deps == ["7"]
    ...
    def test_step_9_is_persona_screen_catalog(self) -> None:
        ...
        assert step.body_template_path == "templates/aas/step-9.md"
    ...
    def test_declaration_order_is_ascending(self) -> None:
        ...
        assert ids.index("7") < ids.index(PERSONA_CATALOG_STEP) < ids.index(PERSONA_SCREEN_STEP)

    def test_dag_waves_end_with_seven_eight_nine(self) -> None:
        ...
        assert tail == [("7",), (PERSONA_CATALOG_STEP,), (PERSONA_SCREEN_STEP,)]
    ...
    def test_gui_layout_rank_is_ascending(self) -> None:
        ...
        assert rank["7"] < rank[PERSONA_CATALOG_STEP] < rank[PERSONA_SCREEN_STEP]


class TestIoContractFiles:
    """scoped I/O contract のファイル名と producer が新しい Step 番号を指すこと。"""

    def test_persona_catalog_contract_uses_step_8(self) -> None:
    ...
    def test_persona_screen_contract_uses_step_9(self) -> None:
...
class TestTemplatesAndPrompts:
    ...
    def test_step_8_template_is_persona_catalog(self) -> None:
    ...
    def test_step_9_template_is_persona_screen(self) -> None:
        body = _read(_TEMPLATES / "aas" / "step-9.md")
        match = self._agent_pattern.search(body)
        assert match is not None
        assert match.group(1).strip() == PERSONA_SCREEN_AGENT
        assert PERSONA_SCREEN_OUTPUT in body
        assert "Step.8" in body, "Step.9 テンプレートは Step.8 への依存を宣言すること"

    def test_persona_catalog_prompt_declares_step_8(self) -> None:
        body = _read(_PROMPTS / f"{PERSONA_CATALOG_AGENT}.prompt.md")
        assert "AAS Step.8 として" in body
        assert "Step.9（ペルソナ別共通画面カタログ）" in body

    def test_persona_screen_prompt_declares_step_9(self) -> None:
        body = _read(_PROMPTS / f"{PERSONA_SCREEN_AGENT}.prompt.md")
        assert "AAS Step.9 として" in body
        assert "AAS Step.8 出力" in body
    ...
    def test_downstream_consumers_point_to_step_9(self, relative_path: str) -> None:
        body = _read(_REPO_ROOT / ".github" / relative_path)
        assert "AAS Step.9 で生成" in body
        assert "AAS Step.8 で生成" not in body


class TestBashRegistryParity:
    """Bash registry が Python registry と同じ Step 8/9 契約を宣言すること。"""
...
class TestPowerShellRegistryParity:
    ...
    def test_powershell_step_8_and_9_match_python_registry(self) -> None:
...
class TestCloudWorkflow:
    """Cloud workflow の skip / 起動 / 完了遷移が 7 → 8 → 9 であること。"""

    def test_skip_propagates_from_step_8_to_step_9(self) -> None:
        body = _read(_CLOUD_WORKFLOW)
        pattern = re.compile(
            r'grep\s+-q\s+" 8 ".*?grep\s+-q\s+" 9 ".*?SKIP_STEPS="\$\{SKIP_STEPS\}\s+9"',
            re.S,
        )
        assert pattern.search(body) is not None, (
            "Step.8 スキップ時に Step.9 を強制スキップする分岐が見つかりません"
        )
        assert 'SKIP_STEPS="${SKIP_STEPS} 8"' not in body, (
            "Step.9 スキップを理由に Step.8 をスキップする旧方向が残っています"
        )

    def test_step_issue_titles_use_new_numbers(self) -> None:
        body = _read(_CLOUD_WORKFLOW)
        assert "[AAS] Step.8: ペルソナカタログ" in body
        assert "[AAS] Step.9: ペルソナ別共通画面カタログ" in body

    def test_step_8_activation_requires_use_case_catalog(self) -> None:
        body = _read(_CLOUD_WORKFLOW)
        pattern = re.compile(
    ...
    def test_step_9_activation_requires_persona_catalog(self) -> None:
        body = _read(_CLOUD_WORKFLOW)
        pattern = re.compile(
            r'activate_with_prereq_check "\$\{S9_NUM\}" "9"(?P<args>(?:\s*\\\s*\n\s*"[^"]+")+)'
        )
        match = pattern.search(body)
        assert match is not None, "Step.9 の起動呼び出しが見つかりません"
...
class TestIssueForm:
    ...
    def test_step_table_rows_use_new_meaning(self) -> None:
        ...
        assert "| **Step.8** | ペルソナカタログ |" in body
        assert "| **Step.9** | ペルソナ別共通画面カタログ |" in body

    def test_dependency_chain_is_ascending(self) -> None:
        body = _read(_ISSUE_FORM)
        assert "Step.7 → Step.8 → Step.9" in body
        assert "Step.9 → Step.8" not in body

    def test_checkbox_labels_declare_new_dependencies(self) -> None:
        body = _read(_ISSUE_FORM)
        assert '- label: "Step.8 — ペルソナカタログ ※ Step.7 が必須"' in body
        assert '- label: "Step.9 — ペルソナ別共通画面カタログ ※ Step.8 が必須"' in body
