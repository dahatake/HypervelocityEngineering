...
_AAS_STEP_DEPENDENCY_PATTERNS: dict[str, None | str | list[str]] = {
    "1":   None,                       # AAS内依存なし（ARD成果物を前提とするroot）
    "2.1": "1",                        # Step.1 に依存
    "2.2": "2.1",                      # Step.2.1 に依存（Step.1.1 は NG）
    "3.1": "2.2",                      # Step.2.2 に依存
    "3.2": "3.1",                      # Step.3.1 に依存
    "4":   "3.1",                      # Step.3.1 に依存（3.2 はサンプルデータのため独立）
    "5":   "4",                        # Step.4 に依存
    "6":   "5",                        # Step.5 に依存
    "7":   "6",                        # Step.6 に依存（ペルソナカタログ）
    "8":   "7",                        # Step.7 に依存（ペルソナ別共通画面カタログ）
}

# aad-web からの混入が疑われる誤ったステップ参照パターン
# 注: 旧 "7": ["Step.5（サービスカタログ）"] は AAS Step.1 起点化により Step.6（テスト戦略書）
# の正しい依存が "Step.5（サービスカタログ）" 自体になったため削除（誤検知を避けるため）。
_FORBIDDEN_DEPENDENCY_PATTERNS = {
    "2.2": ["Step.1.1"],
    "3.1": ["Step.1.2"],
    "4":   ["Step.2（データモデル作成）", "Step.2（"],
    "5":   ["Step.4（画面一覧"],
}
