    ...
    def _parse_structured_questions(content: str) -> List[QAQuestion]:
        ...
        for i in range(1, len(blocks), 2):
            ...
            for line in body.splitlines():
                ...
                val = _match_field(stripped, "質問内容")
                if val is not None:
                    if not q.question:
                        q.question = val
                    collecting_choices = False
                ...
                val = _match_field(stripped, "問題種別")
                if val is not None:
                    if not q.category:
                        q.category = val
                    collecting_choices = False
    ...
    def find_qa_files(qa_dir: Path, pattern: str = "*.md") -> List[Path]:
        ...
        return sorted(
            p for p in qa_dir.glob(pattern)
                if not p.name.endswith("-consolidated.md") and "-workiq-" not in p.name
        )
