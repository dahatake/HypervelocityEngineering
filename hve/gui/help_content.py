...
WORKFLOW_GUIDE_MAP: Dict[str, str] = {
    ...
    "ada": "09-agent-data-architecture.md",
    ...
    "aag": "08-ai-agent.md",
    "aagd": "08-ai-agent.md",
    "aar": "agentic-retrieval-guide.md",
    "akm": "km-guide.md",
...
_WORKFLOW_SHORT: Dict[str, str] = {
    "ard": QT_TRANSLATE_NOOP("help_content", "事業分析〜APP別要求定義（5グループ）。Step 1: 企業の事業分析 / Step 2: 要求定義書作成 / Step 3: KPI/OKR 定義 / Step 4: ユースケース作成 / Step 5: アプリケーション要求定義。既定で Step 2〜5 が ON（Step 1 は明示的に有効化）。"),
    "aas": QT_TRANSLATE_NOOP("help_content", "アプリケーション設計（Step.1〜Step.7）。アーキテクチャ候補を分析し最適構成を選定します。"),
    "ada": QT_TRANSLATE_NOOP("help_content", "画面を持たないデータ中心の AI Agent 向けに、データカタログ・API 詳細・非構造化データ資産を設計します。AAG の前段に実行します。"),
    ...
    "aar": QT_TRANSLATE_NOOP("help_content", "Agentic Retrieval の実装設計・テスト・デプロイ・実測評価を実施します。"),
...
_OPTIONS_FALLBACK: Dict[str, str] = {
    ...
    "app_ids": QT_TRANSLATE_NOOP("help_content", "対象アプリケーション (APP-ID) — カンマ区切りで複数指定可。AAD-WEB/ASDW-WEB/ADFD/ADFDV で共通使用。"),
