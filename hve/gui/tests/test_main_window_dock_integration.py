...
def test_docks_initially_hidden_by_default(main_window) -> None:
    """既定設定で Dock は非表示（MainWindow 最小幅を抑制するため）。"""
    assert main_window._file_tree_dock.isVisible() is False
    assert main_window._preview_dock.isVisible() is False


def test_activity_bar_buttons_match_dock_visibility(main_window) -> None:
    """初期状態でトグルボタンの checked が Dock visibility と一致する。"""
    assert main_window._activity_bar.btn_explorer.isChecked() is False
