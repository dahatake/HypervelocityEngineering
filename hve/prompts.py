...
# DEPRECATED MIGRATION NOTICE (R3 / 2026-05-20)

本モジュールの PROMPT 定数群は **段階的に `hve.prompt_templates` の 9 区分ビルダ関数
への移行**が進行中です（R3 タスク: `work/Issue-orchestration-refactor/remaining/R03/`）。

- 旧定数: 本モジュールの `*_PROMPT` 定数群 18 件
- 新ビルダ: `hve.prompt_templates.build_<bucket>()` （9 区分）
- 削除タイミング: 全 caller の新ビルダ移行完了後、次マイナーリリース（R3.5 で関係者
  合意のうえ実施）。それまでは後方互換のため本モジュールの定数は維持されます。

新規コードは可能な限り `hve.prompt_templates` のビルダ関数を使用し、ドメイン固有の
本文と組み合わせて PROMPT を構築してください。
"""
...
# DEPRECATED: 本ファイル内の `*_PROMPT` 定数は R3.5（次マイナーリリース）で削除予定。
# 新規コードは `hve.prompt_templates.build_<bucket>()` を使用してください。
...
# NOTE: AQOD_PROMPT (Original Docs Review 質問票生成プロンプト) は実装側 caller が
# 存在しない死コードであったため、R3.5 (2026-05-20, work/Issue-orchestration-refactor/
# remaining/R03/) で削除済み。AQOD ワークフロー (workflow_id=="aqod") は AQOD_QA_PROMPT
# (下記) を使用する。
