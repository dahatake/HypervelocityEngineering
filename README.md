...
ビジネス分析と開発のインターフェースとなる要求定義を `knowledge/` ディレクトリ（D01〜D21）に一元集約し、3 つの情報源（`original-docs/` 既存資料・`qa/` 質問票・`src/` 既存コード）から反復的に精緻化します。詳細は [users-guide/overview.md](./users-guide/overview.md#本リポジトリの中核的特徴--knowledge-を介した要求定義の一元管理) を参照してください。
...
graph TD
    ...
    B["02. アプリケーションアーキテクチャ設計"]
    C1["03/05 Microservice<br>設計 → 実装"]
    C2["04/06 Batch<br>設計 → 実装"]
    C4["07/08 AI Agent<br>設計 → 実装"]
...
> 本リポジトリの中核である `knowledge/` 一元管理の全体像は [users-guide/overview.md](users-guide/overview.md#本リポジトリの中核的特徴--knowledge-を介した要求定義の一元管理) を参照してください。

- **方式 1: Copilot cloud agent 手動実行** — GitHub.com 上で Issue を個別に作成し、Custom Agent を選んで Copilot をアサインして動作させる
- **方式 2: ワークフローオーケストレーション（Web）** — Issue Template から親 Issue を作成し、GitHub Actions で Sub Issue を一括生成して自動実行
- **方式 3: ワークフローオーケストレーション（ローカル）** — ローカルの PC / Mac 上で Python SDK を実行

> 全体像と各方式の詳細比較は [users-guide/overview.md](users-guide/overview.md) を参照してください。

### 比較表

| 項目 | 方式 1: Copilot cloud agent 手動実行 | 方式 2: ワークフローオーケストレーション（Web） | 方式 3: ワークフローオーケストレーション（ローカル） |
|------|------|------|------|
| Agent 実行場所 | GitHub Actions | GitHub Actions | ローカル PC |
| Issue 作成 | 手動（1 Issue ずつ） | Issue Template から自動生成 | オプション（デフォルト: しない） |
| Copilot アサイン | 手動 | 自動（`COPILOT_PAT` 必要） | しない（ローカル直接実行） |
| 並列実行 | なし | GitHub Actions 並列ジョブ | 同時実行数を制限（デフォルト: 15） |
...
### 方式 1: Copilot cloud agent 手動実行

GitHub.com 上で Issue を 1 つ作成し、Custom Agent を選択して Copilot をアサインする最もシンプルな使い方です。単一タスクの試行やデバッグに適しています。

> 詳細 → [users-guide/web-ui-guide.md](users-guide/web-ui-guide.md)

### 方式 2: ワークフローオーケストレーション（Web）

Issue Template から親 Issue を作成すると、Bootstrap Workflow が Sub Issue を自動生成し、Copilot が依存関係順に自動実行されます。GitHub Actions を使ったフル自動実行方式です。

> 詳細 → [users-guide/web-ui-guide.md](users-guide/web-ui-guide.md)

### 方式 3: ワークフローオーケストレーション（ローカル）

ローカル環境から Python スクリプトでワークフローを実行します。GitHub Actions を使わずに実行できます。
...
3. **MCP 設定** — Settings → Copilot → Cloud agent → MCP Servers に設定を追加
...
| **QA Original Docs（AQOD）** | [qa-original-docs.md](users-guide/qa-original-docs.md) | `aqod` |
...
| [qa-original-docs.md](users-guide/qa-original-docs.md) | QA Original Docs: `original-docs/` から質問票を `qa/` に生成 |
...
│   ├── qa-original-docs.md         ← QA Original Docs（AQOD）
...
│   ├── workflows/                  ← GitHub Actions ワークフロー定義（32 個）
