...
本リポジトリのワークフローは **3 通りの方法** で実行できます。

- **方法 1: Copilot cloud agent 手動実行** — GitHub.com 上で Issue を個別に作成し、Custom Agent を選んで Copilot をアサインして動作させる
- **方法 2: ワークフローオーケストレーション（Web）** — Issue Template から親 Issue を作成し、GitHub Actions で Sub Issue を一括生成して自動実行
- **方法 3: ワークフローオーケストレーション（ローカル）** — ローカルの PC / Mac 上で Python SDK を実行

> 全体像と各方式の詳細比較は [users-guide/overview.md](users-guide/overview.md) を参照してください。

### 比較表

| 項目 | 方法 1: Copilot cloud agent 手動実行 | 方法 2: ワークフローオーケストレーション（Web） | 方法 3: ワークフローオーケストレーション（ローカル） |
|------|------|------|------|
...
| [web-ui-guide.md](users-guide/web-ui-guide.md) | 方法 1 / 方法 2: Web UI 方式の利用手順・Custom Agent 一覧 |
| [sdk-guide.md](users-guide/sdk-guide.md) | 方法 3: GitHub Copilot CLI SDK 版ユーザーガイド |
...
│   ├── web-ui-guide.md             ← 方法1/方法2: Web UI 方式の利用手順
│   ├── sdk-guide.md                ← 方法3: GitHub Copilot CLI SDK 版ユーザーガイド
...
│   ├── workflows/                  ← GitHub Actions ワークフロー定義（24 個）
