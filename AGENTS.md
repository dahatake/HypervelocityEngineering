...
次のいずれかに当てはまる場合、実装開始前に `work/Issue-<番号>/plan.md` を作る：
...
- `work/Issue-<番号>/plan.md`：DAG+見積+リスク+検証計画
- `work/Issue-<番号>/subissues.md`：Sub Issue の本文（下記の機械可読フォーマットで記述）
- `work/Issue-<番号>/work-status.md`：各ステップの進捗（✅/⏭️/❌）
- `work/Issue-<番号>/README.md`：作業ディレクトリの入口
...
`work/Issue-<識別子>/`（`<識別子>` は対応する GitHub Issue 番号を基本とし、Issue 番号が不明な場合は、作業開始前にわかる最小の識別子を以下の優先順で使用する：① 関連 GitHub Issue 番号、② 関連 PR 番号（既存のものがある場合）、③ `YYYYMMDD-<短い英数字タスク名>` 形式の日付プレフィックス）
...
> **§4 との関係**: §4 の `work/Issue-<番号>/` は Issue 単位の作業ディレクトリ（計画・分割・共有用）。§7.3 の `work/<Custom Agent Name>/<Issue Number>/` は Custom Agent のレビュー出力先であり、別の用途として並立する。なお、`<Issue Number>` は §4 の `<番号>` と同じ値（通常は GitHub Issue 番号だが、Issue 番号不明時は PR 番号や `YYYYMMDD-...` など §4 で定義したフォールバックを含む）を指す。
