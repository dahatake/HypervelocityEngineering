...
> 目的：データフロー処理要件定義で特定した各データフロー処理のデータの流れ（ソース→変換→シンク）を **一貫した粒度**で定義し、Mermaid 図で可視化する。

---

## 使い方（必読）
1. 成果物 `docs/dataflow/data-flow.md` は、このテンプレを **コピーして**作成する。
...
### 必須
- 全データフロー処理を1つの Mermaid 図に集約した概要図
- データソース → データフロー処理 → データシンク の流れを可視化
...
### 根拠
- docs/dataflow/dataflow-requirements.md
