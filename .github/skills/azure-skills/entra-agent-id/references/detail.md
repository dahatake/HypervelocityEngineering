...
For the two-step `fmi_path` exchange (parent token → per-Agent-Identity Graph token) that gives each agent instance a distinct `sub` claim and audit trail, see [runtime-token-exchange.md](runtime-token-exchange.md).

For OBO (agent acting on behalf of a user), see [obo-blueprint-setup.md](obo-blueprint-setup.md).

For the containerized polyglot auth sidecar (Python, Node, Go, Java — no SDK embedding), see [sdk-sidecar.md](sdk-sidecar.md).

For MI+WIF and client-secret setup details, see [oauth2-token-flow.md](oauth2-token-flow.md).
...
See [runtime-token-exchange.md](runtime-token-exchange.md) for full cross-tenant examples.
...
| File | Contents |
|------|----------|
| [runtime-token-exchange.md](runtime-token-exchange.md) | Two-step `fmi_path` exchange: autonomous + OBO, cross-tenant |
| [oauth2-token-flow.md](oauth2-token-flow.md) | MI + WIF (production) and client secret (local dev) |
| [obo-blueprint-setup.md](obo-blueprint-setup.md) | Configuring the Blueprint as an OAuth2 API for OBO |
| [sdk-sidecar.md](sdk-sidecar.md) | Microsoft Entra SDK for AgentID — architecture, configuration, endpoints |
| [sdk-sidecar-deployment.md](sdk-sidecar-deployment.md) | SDK code patterns (Python/TypeScript), Docker/Kubernetes manifests, security, troubleshooting |
| [known-limitations.md](known-limitations.md) | Documented gaps organized by category |
