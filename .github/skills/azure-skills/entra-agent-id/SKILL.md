...
name: entra-agent-id
description: >
  Provision Microsoft Entra Agent Identity Blueprints, BlueprintPrincipals, and per-instance Agent Identities via Microsoft Graph, and configure OAuth 2.0 token exchange (fmi_path, OBO, cross-tenant) including the Microsoft Entra SDK for AgentID sidecar. USE FOR: Agent Identity Blueprint, BlueprintPrincipal, agent OAuth. DO NOT USE FOR: standard Entra app registration (use entra-app-registration). WHEN: Provision Microsoft Entra Agent Identity Blueprints, BlueprintPrincipals, and per-instance Agent Identities via Microsoft Graph, and configure OAuth 2.0 token exchange (fmi_path, OBO, cross-tenant) including the Microsoft Entra SDK for AgentID sidecarが必要なとき。
license: MIT
metadata:
  author: Microsoft
  version: 1.0.1
...
## 目的
- Microsoft Entra Agent ID の適用判断と実行フローを定義する。
