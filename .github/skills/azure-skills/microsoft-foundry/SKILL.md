...
description: >
  Deploy, evaluate, and manage Foundry agents end-to-end: Docker build, ACR push, hosted/prompt agent create, container start, batch eval, continuous eval, prompt optimizer workflows, agent.yaml, dataset curation from traces. USE FOR: deploy agent to Foundry, hosted agent, create agent. DO NOT USE FOR: Azure Functions. WHEN: Deploy, evaluate, and manage Foundry agents end-to-end: Docker build, ACR push, hosted/prompt agent create, container start, batch eval, continuous eval, prompt optimizer workflows, agent.yaml, dataset curation from tracesが必要なとき。
license: MIT
metadata:
  author: Microsoft
  version: 1.1.9
...
## 目的
- Microsoft Foundry Skill の適用判断と実行フローを定義する。
