# Phase別コマンドリファレンス・検証レポート・エスカレーション

> 本ファイルは `harness-verification-loop/SKILL.md` の §1〜§3 詳細を収容する参照資料です。
...
- Shell: `#!/bin/bash` シバン確認、`[[` 等の bash 固有構文の誤用確認
...
**失敗時の対処**:
- C#: `dotnet format` を実行して自動修正
- Python: `ruff check --fix .` で自動修正
...
- Error: {エラーメッセージ}
- File:  {ファイルパス:行番号}
- Action: {対処内容または harness-error-recovery 参照}
```
...
2. Skill `harness-error-recovery` の3要素（`root_cause_hint` / `safe_retry_instruction` / `stop_condition`）を出力する
3. 修正を試みる（最大3回: copilot-instructions.md §0 リトライルール準拠）
