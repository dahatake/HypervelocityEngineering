...
description: >
  タスクの依存関係（DAG）で分解し、タスク粒度とコンテキストサイズで分割判断を行うスキル。
  plan.md / subissues.md を作成し、task_scope=multi または context_size=large のタスクを安全に分割する。
  USE FOR: create plan, estimate task, split issue, DAG planning,
  dependency analysis, plan.md creation, subissues.md generation,
  task_scope/context_size threshold check, SPLIT_REQUIRED decision, PROCEED decision.
  ...
  DAG を作成、依存関係を整理、task_scope=multi または context_size=large の作業、
