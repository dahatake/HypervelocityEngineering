> テスト仕様書（docs/test-specs/{agentId}-test-spec.md）に基づき、TDD RED フェーズのテストコード（失敗するテスト）を src/test/agent/{AgentName}.Tests/ 配下に生成する。実装コードは作成しない。
...
# 3) 出力（成果物）
必須:
- `src/test/agent/{AgentName}.Tests/` 配下にテストコード
...
任意だが推奨:
- `src/test/agent/{AgentName}.Tests/README.md`（テストの実行方法・前提条件・RED 状態の説明）
...
# 8) 完了条件（DoD）
- `src/test/agent/{AgentName}.Tests/` 配下にテストプロジェクトが存在し、ビルドが成功する。
