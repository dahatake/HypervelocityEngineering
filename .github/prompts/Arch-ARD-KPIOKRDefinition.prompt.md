> 事業要件文書の戦略的記述を根拠に KPI/OKR、計測データ定義、目的志向のデータ収集設計を作成する（ARD Step 3・任意）。USE FOR: KPI/OKR definition, measurement data design, data collection strategy. WHEN: ARD Step 3 を実行するとき、include_kpi_okr=true が指定されたとき。
