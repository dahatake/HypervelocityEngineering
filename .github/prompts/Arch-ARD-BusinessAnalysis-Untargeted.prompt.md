> 対象事業が未定の段階で企業全体の事業分析を行い、経営陣向け事業分析レポートを作成する。
...
- ARD ワークフローの **Step 1（事業分野候補列挙）/ 1.1（per-BIZ-NN fan-out）/ 1.2（事業分析統合）** に再利用される。出力先は Step ごとに異なるため、§3 のテーブルを参照すること。

## 2) 入力（必ず参照）
- 本ファイル **§4 Prompt 本文** を出力仕様の Single Source of Truth とする。外部ガイドを読みに行かないこと。
- 分析対象は企業全体（対象事業未定）とする。
- **添付資料・指定資料（`attached_docs` / パス指定の `target_business`）が与えられている場合は、それを一次情報として最優先で参照する。** 資料に記載のある事実は必ずその内容を優先し、資料に記載が無い事項のみを公開情報で補う。これらのファイルは `required_input_paths` として宣言されず、パラメータ経由でのみ渡されるため、ファイル名を推測せず与えられたパスをそのまま読むこと。
- 添付資料・指定資料が無い場合（`添付なし`）は、公開情報の一次資料を優先する。
...
| 1.1 | 事業分野別深掘り（per-BIZ-NN fan-out） | `docs/business/{key}-analysis.md`（例: `docs/business/BIZ-01-analysis.md`）|
| 1.2 | 事業分析統合 | `docs/company-business-requirement.md` |

- ファイル名 `recommendation.md`（Step 1）と `requirement.md`（Step 1.2）を取り違えてはならない。
- Step 1.1 では `docs/business/` 以外の場所に per-key ファイルを作成してはならない。
...
```text
# 最終出力
- 出力ファイルは §3 の Step 別テーブルに従う（Step 1: `docs/company-business-recommendation.md` / Step 1.1: `docs/business/{key}-analysis.md` / Step 1.2: `docs/company-business-requirement.md`）。Agent 側で出力ファイルを推測・変更してはならない。他の Step の出力ファイルは生成しない。

# Step 別スコープ（本文の適用範囲）
以降の「出力形式」（第 1〜7 章の全社レポート）は **Step 1.2 専用** である。実行中の Step に応じて適用範囲を切り替えること。
- **Step 1（事業分野候補列挙）**: `| BIZ-NN | 名称 | 概要 | 想定規模 |` の候補表を中心とした簡潔な文書のみを出力する。候補数は通常 5〜10 件、上限 10 件。**第 1〜7 章の全社レポートを生成してはならない**。各候補の詳細分析は Step 1.1 へ、全社統合分析は Step 1.2 へ委譲する。
- **Step 1.1（事業分野別深掘り）**: 指定された 1 つの `BIZ-NN` に限定して分析する。他の候補への言及は最小限にする。
- **Step 1.2（事業分析統合）**: 以降の「出力形式」の第 1〜7 章構成を完備した全社レポートを出力する。
...
# 参照資料
- 添付資料: {添付資料}（**提供がある場合は一次情報として最優先で参照する**。`添付なし` の場合は公開情報のみで分析）
...
# 情報源の優先順位
**添付資料・指定資料（{添付資料}）が提供されている場合は、それを一次情報として最優先で参照し、記載のある事実は以下の公開情報より優先してください。**

添付資料に記載が無い事項については、以下の順で信頼性を評価してください。
