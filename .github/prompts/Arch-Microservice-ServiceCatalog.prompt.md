> 画面→機能→API→SoTデータのマッピングを docs/catalog/service-catalog-matrix.md に生成/更新

> **WORK**: `work/run/<run-id>/Arch-Microservice-ServiceCatalog/Issue-<識別子>/`
...
| サービス名 | 種別 | 利用APP | SoTデータ（所有） | 主要責務（1行） | 依存（参照のみ等） | スケジュール/DAG（種別=非同期ジョブのみ） | リトライ戦略（種別=非同期ジョブのみ） | 出典(ファイル#見出し) |
|---|---|---|---|---|---|---|---|---|

> **列の説明**:
> - `種別`: `同期API` / `非同期ジョブ`（バッチ・データフロー処理）/ `TBD`。Service Identify Agent の §B 詳細「種別」と一致させる。
> - `スケジュール/DAG`: 非同期ジョブのみ記載。例: `daily 03:00 JST` / `event: SVC-09 完了後` / `cron: 0 */6 * * *`。同期 API は `—`。
> - `リトライ戦略`: 非同期ジョブのみ記載。例: `指数バックオフ 3回` / `DLQ 退避` / `手動再実行`。同期 API は `—`。**§6 の冪等性/競合制御注意点と矛盾しない範囲で記載**。不明は `TBD`。
...
- **ユーザー視点・トレーサビリティ**：Table A の API が Table B に存在 / サービス名が Table C と一致 / 関係が矛盾していないか
