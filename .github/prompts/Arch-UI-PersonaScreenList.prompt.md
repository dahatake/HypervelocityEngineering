...
## 1) このエージェントの目的
AAS Step.8 として、`docs/catalog/persona-catalog.md`（AAS Step.9 出力）と `docs/catalog/app-catalog.md` を根拠に、**APP-ID を横断して共通化できるペルソナ別画面の骨格** を一覧化する。
...
## 2) 入力（読む順序）
最優先：
- `docs/catalog/persona-catalog.md`（**必須**。AAS Step.9 出力。本 Agent はここで定義済みの persona_id のみ扱う。**存在しない場合は成果物生成を停止し、進捗ログに `BLOCKED: persona-catalog.md 不在` と理由を記録する**）
