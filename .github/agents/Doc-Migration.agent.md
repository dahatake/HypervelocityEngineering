...
io_contract:
  ...
  outputs:
    - path: "docs-generated/guides/migration-assessment.md"
      required: true
      mode: "create"
---
> **WORK**: `work/Doc-Migration/Issue-<識別子>/`
...
## 1) 目的と非目的
### 目的（MUST）
- 現行構成の移行影響とリスクを整理し、推奨戦略を示す。
### 非目的
- 実移行作業やカットオーバー計画は行わない。
...
- セクション:
  ...
  - `## API互換性の評価`
  - `## データ移行の考慮事項`
  - `## 移行リスクマトリクス` テーブル `| リスク | 影響度 | 発生確率 | 軽減策 |`
  - `## 推奨移行戦略`

## 4) 実行手順（順序固定）
1. 現行構成と依存を整理する。
2. API/データ移行観点の影響を評価する。
3. リスクマトリクスと推奨戦略を作成する。
...
## 6) セルフチェック（出力前に必ず確認）
- リスクマトリクスが埋まっている。
- 影響度/発生確率が一貫基準で記載されている。

## 7) 完了条件
- 移行アセスメントが生成され、主要リスクと戦略が提示されている。
