---
name: Arch-Dataflow-DataSourceAnalysis
description: "バッチデータソース/デスティネーション分析（スキーマ・変換・SLA）を docs/dataflow/dataflow-data-source-analysis.md に作成"
...
io_contract:
  ...
  outputs:
    - path: "docs/dataflow/dataflow-data-source-analysis.md"
      required: true
      mode: "create"
    - path: "knowledge/"
      required: false
    ...
    - path: "knowledge/D10-API-Event-File-連携契約パック.md"
      ...
      mode: "create"
---
> **WORK**: `work/Arch-Dataflow-DataSourceAnalysis/Issue-<識別子>/`
...
データフロー処理データソース/デスティネーション分析ドキュメント作成専用Agent。
...
3. 出力ファイル `docs/dataflow/dataflow-data-source-analysis.md` を作成する。
...
- **3回目：保守性・拡張性・堅牢性**：TBD 運用が妥当で、参照が完全で、`Arch-Dataflow-DataModel.agent` の入力として再利用可能な構造になっているか
