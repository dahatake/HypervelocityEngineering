---
name: Dev-Dataflow-DataServiceSelect
description: "バッチサービスカタログを読み、必要な Azure データサービスを特定して infra/azure/dataflow/ スクリプトを作成する（Step 1.1: データサービス選定）"
tools: ["*"]
metadata:
  version: "1.0.0"
...
- `infra/azure/dataflow/create-batch-resources.sh`（Azure CLI でデータフロー用リソースを冪等作成）
- `infra/azure/dataflow/verify-batch-resources.sh`（作成したリソースの存在・状態検証）
- 作業ログ: `{WORK}` 配下
...
```
A) Azure 作成スクリプト作成（infra/azure/dataflow/）
→ E) 進捗ログ（随時更新）
```
...
2. `infra/azure/dataflow/create-batch-resources.sh` を作成する（`azure-cli-deploy-scripts` Skill §1「3点セットテンプレート」および §2「冪等性パターン」に準拠）：
...
3. `infra/azure/dataflow/verify-batch-resources.sh` を作成する（`azure-cli-deploy-scripts` Skill §1.4 テンプレートに準拠）：
