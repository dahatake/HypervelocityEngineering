---
name: Dev-Dataflow-TestCoding
description: "バッチTDDテスト仕様書に基づきTDD REDフェーズのテストコードを test/dataflow/{jobId}-{jobNameSlug}.Tests/ に生成（実装コード不可）"
tools: ["*"]
metadata:
  version: "1.0.0"
...
io_contract:
  inputs:
    - path: "docs/test-specs/{jobId}-test-spec.md"
      required: true
      kind: "agent_artifact"
      producer: "Arch-Dataflow-TDD-TestSpec"
    - path: "docs/dataflow/dataflow-test-strategy.md"
      required: true
      kind: "agent_artifact"
      producer: "Arch-Dataflow-TestStrategy"
    - path: "docs/dataflow/apps/{jobId}-{jobNameSlug}-spec.md"
      required: true
      kind: "agent_artifact"
      producer: "Arch-Dataflow-AppSpec"
    - path: "docs/dataflow/dataflow-data-model.md"
      required: true
      kind: "agent_artifact"
      producer: "Arch-Dataflow-DataModel"
  ...
  outputs:
    - path: "test/dataflow/{jobId}-{jobNameSlug}.Tests/"
      required: false
      mode: "create"
    - path: "test/dataflow/{jobId}-{jobNameSlug}.Tests/README.md"
      required: true
      mode: "create"
    - path: "{WORK}"
      required: true
...
- `test/dataflow/` ディレクトリ構造（既存テストコードのパターン確認）

### 3.3 出力（必須）

- `test/dataflow/{jobId}-{jobNameSlug}.Tests/` 配下にテストコード（xUnit + C# を既定とする。既存テストプロジェクトの慣習があればそれに従う）
...
- `test/dataflow/{jobId}-{jobNameSlug}.Tests/README.md`（テストの実行方法・前提条件・RED 状態の説明）
- 作業ログ: `{WORK}` 配下
...
- 既存の `test/dataflow/` 配下にテストプロジェクトがあれば、構成・フレームワーク・モッキングライブラリ・命名規則の"型"を踏襲する。
...
- ただし、コンパイル/ビルドを通すために必要な最小限のインターフェース定義やスタブクラスは作成してよい（`src/dataflow/` 配下ではなく `test/dataflow/` 配下に配置すること）。
...
- `test/dataflow/{jobId}-{jobNameSlug}.Tests/` 配下にテストプロジェクトが存在し、`dotnet build` が成功する。
