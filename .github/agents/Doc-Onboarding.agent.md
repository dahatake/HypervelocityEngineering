---
name: Doc-Onboarding
description: "新規参画者向けオンボーディングガイドを生成する。"
tools: ["*"]
metadata:
  version: "1.0.0"
...
io_contract:
  ...
  outputs:
    - path: "docs-generated/guides/onboarding.md"
      required: true
      mode: "create"
---
...
## 1) 目的と非目的
### 目的（MUST）
- 初学者が短時間で全体像を把握できるガイドを作成する。
### 非目的
- 開発ルールの制定や教育計画策定は行わない。
...
## 4) 実行手順（順序固定）
1. 既存ドキュメントから必須情報を抽出する。
2. 初学者向けに平易化して再構成する。
3. セットアップ/読み方/FAQを作成する。
...
## 7) 完了条件
- オンボーディングガイドが生成され、初期学習の入口として利用可能。
