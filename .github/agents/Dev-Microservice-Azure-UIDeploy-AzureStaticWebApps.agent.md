...
4. Azure が自動生成した workflow YAML のカスタマイズ（`.github/workflows/azure-static-web-apps-*.yml`）
   ...
   - **注意**: workflow のトリガー設定を以下のとおり確認・修正する：
    ...
    - **`workflow_dispatch` トリガーの追加（Copilot Workflow 実行制限回避策）**:
      - Copilot Coding Agent が push したコミットでは GitHub Actions Workflow が自動実行されない
        （GitHub セキュリティ制約: GITHUB_TOKEN による push は後続 Workflow を発火しない）
      - 回避策として `workflow_dispatch` トリガーを追加し、GitHub Actions タブから手動実行可能にする
...
Copilot Coding Agent が push したコミットでは GitHub Actions Workflow が自動実行されない。
以下の手順を PR description に記録し、ユーザーに案内すること：
...
2. **フォールバック: Actions タブから手動実行**
   - GitHub リポジトリの Actions タブ → 「Azure Static Web Apps CI/CD」Workflow を選択
...
### AC 検証結果の記録先
`{WORK}ac-verification.md` に以下のフォーマットで記録する：
...
| # | AC | 判定 | エビデンス要約 |
|---|-----|------|---------------|
| AC-1 | Azure リソース存在確認（最重要） | ✅ / ❌ / ⏳ | defaultHostname の値 or 手動検証待ち |
| AC-2 | create.sh 冪等性 | ✅ / ❌ / ⏳ | コードレビュー結果・実行結果 |
| AC-3 | Workflow: Azure/static-web-apps-deploy@v1 + app_location: app | ✅ / ❌ | grep 結果 |
| AC-4 | service-catalog 記載 | ✅ / ❌ | URL or 取得手順の記載確認 |
| AC-5 | 秘密情報なし | ✅ / ❌ | grep 結果（該当なし or 検出内容） |
| AC-6 | 作業ブランチからのデプロイ成功（Secret 自動設定含む） | ✅ / ❌ / ⏳ | az staticwebapp show 結果 or 手動検証待ち |
| AC-7 | switch-swa-to-main.sh 存在・冪等性 | ✅ / ❌ | コードレビュー結果（スキップロジック・update コマンド） |
...
if AC-1 == ❌:
    ...
    判定 = "条件付き完了"
    → 最終品質レビューに進んでよい
    → PR description に「⚠️ 手動検証待ちの AC あり」を明記
