...
> 具体的には、入力ファイルなしのAPPは **A) サマリ表（全APP横断）** に含め、**C) 未処理・不足APP一覧（厳守）には含めない**。また、処理統計では「判定未完了」に算入せず、**デフォルト適用件数** として計上する。
...
> **⚠️デフォルト適用（入力ファイルなし）のAPPは「処理済み」扱いであるため、このセクションには含めない（A) サマリ表（全APP横断）に記録する）。**
