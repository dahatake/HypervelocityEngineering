...
"全ジョブ対応""設計刷新""横断リファクタ"は範囲外（必要なら AGENTS.md の分割ルールで別タスク化）。
...
- 実装コードは「ファイル単位」で段階的に書く（エントリポイント → 変換ロジック → バリデーション → エラーハンドリング → テスト更新）。
...
- `task-dag-planning`: 計画・分割が必要な場合（変換ロジックが複雑、15分超の見込みがある場合）に参照する。
