---
name: Doc-TestSummary
description: "テストコード1ファイルのテスト対象・ケース・モック構成を要約する。"
tools: ["*"]
metadata:
  version: "1.0.0"
...
io_contract:
  ...
  outputs:
    - path: "docs-generated/files/{relative-path}.md"
      required: true
      mode: "create"
    - path: "| テスト名 | 種別(unit/integration/e2e) | テスト対象 | アサーション概要 |"
      required: true
      mode: "create"
---
...
- セクション:
  - `## テスト対象`
  - `## テストケース一覧` テーブル `| テスト名 | 種別(unit/integration/e2e) | テスト対象 | アサーション概要 |`
  - `## モック・スタブ` テーブル `| 対象 | モック手法 | 理由 |`
  - `## カバレッジ観察（推定）`

## 4) 実行手順（順序固定）
1. テスト対象コード/モジュールを特定する。
2. テスト関数ごとにケースを抽出する。
3. モック/スタブを抽出する。
4. 観察可能な範囲でカバレッジ傾向を記述する。
...
## 6) セルフチェック（出力前に必ず確認）
- テストケース一覧が空でない。
- 種別が unit/integration/e2e のいずれかで記載されている。
- モックの理由が記載されている。

## 7) 完了条件
- 出力ファイルが生成され、主要テストケースの漏れがない。
