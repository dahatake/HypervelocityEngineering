---
name: Arch-Dataflow-TestStrategy
description: "データフロー処理テスト戦略書（冪等性・データ品質・障害注入）を docs/dataflow/dataflow-test-strategy.md に作成"
...
io_contract:
  inputs:
    ...
    - path: "docs/dataflow/dataflow-app-catalog.md"
      required: false
      kind: "agent_artifact"
    - path: "docs/dataflow/dataflow-domain-analytics.md"
      required: false
      kind: "agent_artifact"
  outputs:
    ...
    - path: "knowledge/D17-品質保証-UAT-受入パッケージ.md"
      ...
      mode: "create"
---
> **WORK**: `work/Arch-Dataflow-TestStrategy/Issue-<識別子>/`
...
- `docs/dataflow/dataflow-app-catalog.md`（存在する場合）
- `docs/dataflow/dataflow-domain-analytics.md`（存在する場合）
...
4. べき等性（再実行耐性）：`docs/dataflow/dataflow-test-strategy.md` は上書き更新（重複作成しない）。`work/` 配下のファイルは Skill work-artifacts-layout §4.1 に従い削除→新規作成とする。

## 4) バッチ固有テスト戦略の作り方（ルール）

- **テストピラミッド**: `test-strategy-template` Skill §1 参照。データフロー処理に適したレイヤー配分（ユニット / 統合 / E2E / バッチ固有）を表形式で定義。各レイヤーの対象・カバレッジ目標・実行タイミングを `batch-service-catalog.md` の Job-ID を根拠に記述する。
- **バッチ固有テスト種別**:
