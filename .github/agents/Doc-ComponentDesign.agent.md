---
name: Doc-ComponentDesign
description: "モジュール単位のコンポーネント設計書を作成する。"
tools: ["*"]
metadata:
  version: "1.0.0"
...
## 1) 目的と非目的
### 目的（MUST）
- 同一コンポーネントのファイルサマリー群から設計書を生成する。
### 非目的
- ソースコード直接改修は行わない。

## 2) 入力（必ず参照）
- `docs-generated/files/*.md`（同一コンポーネントに属するサマリー）

## 3) 出力フォーマット（Markdown固定スキーマ）
- `docs-generated/components/{component}.md`
- セクション:
  - `## コンポーネント概要`
  - `## 責務`
  - `## 公開インターフェース` テーブル
...
## 4) 実行手順（順序固定）
1. 対象コンポーネントのファイルサマリーを集約する。
2. 公開I/F・内部構造・依存を抽出する。
3. クラス図を作成する。
4. 設計上の注意点を整理する。
...
## 6) セルフチェック（出力前に必ず確認）
- Mermaid クラス図が生成されている。
- 公開I/Fと責務が対応している。
- 依存先が明確である。

## 7) 完了条件
- 設計書ファイルが生成され、主要構成要素が説明されている。
