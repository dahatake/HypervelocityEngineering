...
7. `batch-test-strategy.md` の「4. テストダブル戦略」から、各依存パターンのテストダブル選択基準（Azurite/Testcontainers/Mock）を抽出する。
8. `batch-test-strategy.md` の「3. テストデータ戦略」からデータ生成方式（Faker/シード管理/サニタイズ）を抽出する。
...
- それでも安定しない場合は分割へ切り替え、`{WORK}subissues.md` を作る（分割粒度: ジョブ単位）。AGENTS.md §4.1 に従い、既存ファイルがある場合は削除してから新規作成する。
