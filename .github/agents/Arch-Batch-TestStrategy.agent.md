...
- **テストピラミッド**: バッチ処理に適したレイヤー配分（ユニット / 統合 / E2E / バッチ固有）を表形式で定義。各レイヤーの対象・カバレッジ目標・実行タイミングを `batch-service-catalog.md` の Job-ID を根拠に記述する。
- **バッチ固有テスト種別**:
...
- **テストデータ戦略**:
  - **生成方式**: Faker（ランダム生成）/ シード管理（再現可能）/ 本番データサニタイズの方針を用途別に選択
  - **ボリューム方針**: ユニットテスト（少量/固定値）、統合テスト（中量）、大量データテスト（本番相当）
  - **エッジケース**: NULL/空文字/境界値/重複レコード/文字化けデータを必ず含める
- **テストダブル戦略**:
  - **データソースモック**: 外部データソースのモック（Stub / Mock / Fake）。Azurite（Azure Blob/Queue/Table Storage エミュレーター）を優先的に採用する。
...
- `batch-test-strategy.md` は「セクション単位」で段階的に書く（テストピラミッド→バッチ固有テスト→テストデータ→テストダブル→…）。
