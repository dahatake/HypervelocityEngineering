...
description: qa/ フォルダーの質問ファイルを読み取り、docs/business-requirement-document-master-list.md の D01〜D21 に分類し、work/business-requirement-document-status.md を生成・更新する。
tools: ["*"]
---
...
## §1 目的
`qa/` 配下の質問ファイルに蓄積された質問項目を、`docs/business-requirement-document-master-list.md` で定義された D01〜D21 の文書クラスに分類し、各文書クラスの充足状況をステータス管理する。
...
- `docs/business-requirement-document-master-list.md` — D01〜D21 の文書クラス定義
...
### Step 2: マスターリスト読み込み
- `docs/business-requirement-document-master-list.md` を読み込む。
...
- 各 D クラスの「不足判定基準」（`docs/business-requirement-document-master-list.md` の `**不足判定:**` フィールド）と現状を照合し、不足箇所を特定する。
...
- **D クラス定義との整合**: `docs/business-requirement-document-master-list.md` の定義と矛盾する分類は禁止。
