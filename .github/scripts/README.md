...
.github/scripts/
├── README.md                   ← このファイル
├── preflight-cloud-setup.sh    ← HVE Cloud Agent Orchestrator 初回設定の事前確認
├── bash/
...
| `MODEL` | Copilot モデル指定（省略時は空文字 = `Auto`。GitHub が最適モデルを動的選択） | 省略可 |
| `DRY_RUN` | `1` で dry-run モード（Bash）| 省略可（`--dry-run` 推奨） |

## 使用例

### Cloud setup preflight（HVE Cloud Agent Orchestrator 初回設定の任意チェック）

HVE Cloud Agent Orchestrator を初めて使う前に、`gh` で確認可能な初期設定漏れをローカルから点検できます。

#### 前提ツール

- GitHub CLI (`gh`)
- `gh auth status` が成功するログイン状態
...
# self-hosted runner label も確認する場合（任意）
bash .github/scripts/preflight-cloud-setup.sh OWNER/REPO --self-hosted-runner-label aca
```

#### チェック内容

- `gh` インストール確認 / `gh auth status` 確認
- `OWNER/REPO` 指定確認 / リポジトリアクセス確認
- workflow 一覧取得、`Setup Labels` workflow の存在確認
- 主要ラベル（`setup-labels`, `auto-app-selection`, `auto-app-detail-design-web`）の存在確認
- secret 名の存在確認（`COPILOT_PAT`, `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`）
- （任意）self-hosted runner label の一致確認
- API で断定できない項目は「手動確認項目」として表示

#### PASS / WARN / FAIL の意味

- `PASS`: 自動チェックで問題なし
- `WARN`: 権限不足・API制約・任意項目未実施・初回状態の可能性あり（要確認）
- `FAIL`: 必須チェック失敗（終了コードは non-zero）

> [!IMPORTANT]
> secret は**名前の存在確認のみ** 行い、値は表示しません。

preflight の失敗パターンや `WARN` / `FAIL` の解釈は [users-guide/troubleshooting.md - Cloud preflight スクリプト](../../users-guide/troubleshooting.md#10-cloud-preflight-スクリプト) を参照してください。
