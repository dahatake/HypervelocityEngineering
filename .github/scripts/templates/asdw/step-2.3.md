{root_ref}
## 目的
`docs/azure/AzureServices-services-additional.md` を根拠に、追加Azureサービスを Azure CLI で冪等に作成する（APP-ID 指定時はスコープ内のサービスのみ）。
...
4. デプロイスクリプトの作成・実行
5. 検証スクリプト実行 → 全 PASS まで修正（最大 3 回反復。超過時は `asdw:blocked` + FAIL 項目一覧を報告）

## Custom Agent
`Dev-Microservice-Azure-AddServiceDeploy` を使用

## 依存
- Step.2.2（追加 Azure サービス選定）が `asdw:done` であること
