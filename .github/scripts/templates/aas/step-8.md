...
本ステップは APP-ID 横断の単一の真実源（SoT）として、複数 APP-ID に登場する同一ペルソナの重複定義を防ぎ、後続 Step.9（ペルソナ別共通画面カタログ）の前提となる。
...
- **ペルソナID 採番ルール**: ユースケースに既存アクターID がある場合は流用する。無い場合は `PRS-001` 形式で暫定採番する（PRS = Persona）。ID は Step.9 等からの参照安定化のためで、新規ペルソナ概念を作らない
