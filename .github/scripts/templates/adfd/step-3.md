...
## 目的
ドメイン分析・データソース分析・バッチデータモデルを根拠に、データフローアプリ設計書（ジョブ一覧・依存DAG・スケジュール・リトライ戦略・冪等性保証）を作成する。
...
## 依存
- Step.2（バッチデータモデル）が `adfd:done` であること

## 完了条件
- `docs/dataflow/dataflow-app-catalog.md` が作成されている
