#!/usr/bin/env bash
# copilot-assign.sh — Copilot cloud agent を Issue にアサインする共通関数
#
# 依存環境変数:
...
#   0 = success
#   1 = failure
#   2 = skipped（例: COPILOT_PAT 未設定）

_COPILOT_ASSIGN_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

extract_model() {
  local body="$1"
  printf '%s' "${body}" | python3 "${_COPILOT_ASSIGN_LIB_DIR}/extract-model.py"
}

extract_review_model() {
  local body="$1"
  printf '%s' "${body}" | python3 "${_COPILOT_ASSIGN_LIB_DIR}/extract-review-model.py"
}
...
assign_copilot() {
  ...
  if [[ "${DRY_RUN:-0}" == "1" ]]; then
    echo "DRY_RUN=1 のため Copilot アサインをスキップします: Issue #${issue_num}"
    return 0
  fi
  ...
  current_assignees=$(curl -s \
    ...
    | python3 "${_COPILOT_ASSIGN_LIB_DIR}/check-assignees.py") || true
  ...
  existing_prs=$(curl -s \
    ...
    | python3 "${_COPILOT_ASSIGN_LIB_DIR}/check-existing-prs.py") || true
...
query(\$issueNumber: Int!) {
  repository(owner: \"${OWNER}\", name: \"${REPO_NAME}\") {
    ...
    IFS=$'\t' read -r bot_id issue_node_id repo_node_id < <(printf '%s' "${query_result}" | python3 "${_COPILOT_ASSIGN_LIB_DIR}/parse-graphql-ids.py") || true
...
mutation(\$assignableId: ID!, \$botId: ID!, \$targetRepositoryId: ID!, \$baseRef: String!, \$customInstructions: String!, \$customAgent: String!) {
  addAssigneesToAssignable(input: {
    ...
    local has_errors
    has_errors=$(echo "${result}" | python3 "${_COPILOT_ASSIGN_LIB_DIR}/check-mutation-errors.py") || true
    ...
    local is_assigned
    is_assigned=$(echo "${result}" | python3 "${_COPILOT_ASSIGN_LIB_DIR}/check-assignment-result.py") || true
